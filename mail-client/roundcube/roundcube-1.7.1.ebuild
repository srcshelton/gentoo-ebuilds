...
RDEPEND="
	${DEPEND}
	>=dev-lang/php-8.1.0[exif?,fileinfo?,filter,gd,iconv,intl,json(+),ldap?,mysql?,pdo,postgres?,session,sockets,sqlite?,ssl?,unicode,xml,zip?]
	virtual/httpd-php
	change-password? (
	...
	managesieve? ( >=dev-php/PEAR-Net_Sieve-1.4.5 )
	mysql? (
		|| (
			dev-lang/php[mysql]
			dev-lang/php[mysqli]
...
src_compile() {
	# roundcube's Makefile is explicitly only there to build release-packages,
	# rather than to build roundcube itself and so we don't actually want to
	# run default_src_compile() and 'emake' here.
	:
}
