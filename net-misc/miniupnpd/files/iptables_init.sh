...
debug "Checking for NAT chain '${CHAIN}-PCP-PEER' ..."
doiptables -t nat -L -nx | grep -q "^Chain ${CHAIN}-PCP-PEER " || {
	debug "Creating NAT chain '${CHAIN}-PCP-PEER'"
	doiptables -t nat -N "${CHAIN}-PCP-PEER"
}
doiptables -t nat -A POSTROUTING -o "${EXTIF}" -j "${CHAIN}-PCP-PEER"
