...
inherit eapi9-ver fcaps flag-o-matic meson python-r1 systemd tmpfiles toolchain-funcs verify-sig
...
LICENSE="MPL-2.0"
SLOT="0"
IUSE="benchmark debug doc examples filecaps kerberos mysql -netconf +openssl postgres +samples +shell systemd test tmpfiles"
...
src_prepare() {
	...
	if use !doc; then
		PATCHES+=( "${FILESDIR}"/${PN}-3.0.0-strip-doc.patch )
	fi
	default
...
src_configure() {
	...
	# patching meson.build to set umask, instead here we pass install-umask as
	# an argument to do the same thing, i.e. control permissions on installed
	# files.
	local emesonargs=(
...
src_install() {
	...
	# so make sure permissions are same as in previous versions of kea to avoid
	# any differences between an update vers first time install
	...
	# A side effect of using install_umask 023 in meson setup is setting config
	# files to be world readable... let's not do that
