...
inherit autotools flag-o-matic pam systemd toolchain-funcs user-info
...
IUSE="abi_mips_n32 audit debug hpn kerberos kernel_linux ldns libedit livecd pam +pie +scp sctp security-key selinux +ssl static systemd test X X509 xmss"
...
DEPEND="${RDEPEND}
	virtual/os-headers
	kernel_linux? ( !prefix-guest? ( virtual/os-headers:50100 ) )
	static? ( ${LIB_DEPEND} )
"
RDEPEND="${RDEPEND}
...
src_prepare() {
	...
	eapply "${FILESDIR}/${PN}-8.6_p1-clientloop.patch"
...
src_configure() {
	...
	if use amd64 && [[ "${ABI}" == 'x32' ]] # && ! use libseccomp
	then
