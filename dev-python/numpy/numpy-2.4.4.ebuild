...
IUSE="big-endian +cpudetection index64 +lapack lto ${ARM_FLAGS[*]/#/cpu_flags_arm_} ${PPC_FLAGS[*]/#/cpu_flags_ppc_} ${X86_FLAGS[*]/#/cpu_flags_x86_}"
...
python_configure_all() {
	...
	case ${ARCH} in
		...
		amd64|x86)
			...
			local mapping=(
				# for these, we just check the newest
				# instruction set, since all earlier
				# instruction sets imply it
				"X86_V2=sse4_2"
				"X86_V3=avx2"
				# from here on, we check all features that were
				# added in the lowest CPU supporting them all
	...
	DISTUTILS_ARGS=(
		...
		-Dcpu-dispatch="$(usex cpudetection MAX none)"
	)
