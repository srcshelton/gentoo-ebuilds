...
IUSE="debug doc +icu kerberos ldap llvm +lz4 nls +numa oauth pam perl python +readline selinux +server ssl static-libs systemd tcl +uring uuid xml zlib +zstd"
...
src_prepare() {
	...
	if use pam ; then
		sed \
				-e "s/\(#define PGSQL_PAM_SERVICE \"postgresql\)/\1-${SLOT}/" \
