...
IUSE="debug doc icu kerberos ldap llvm lz4 nls pam perl python +readline selinux +server systemd ssl static-libs tcl threads uuid xml zlib zstd"
...
src_prepare() {
	...
	sed "s|\(PGSOCKET_DIR\s\+\)\"/tmp\"|\1\"${EPREFIX}/var/run/postgresql\"|" \
...
src_install() {
	...
	local f bn
	for f in $(
		find "${ED}/usr/$(get_libdir)/postgresql-${SLOT}/bin" -mindepth 1 -maxdepth 1
	)
	do
		bn=$(basename "${f}")
	...
	if use server; then
		...
		if use prefix ; then
			...
			fperms 1775 /var/run/postgresql
		fi
	fi
}
