...
inherit bash-completion-r1 meson optfeature perl-module plocale python-single-r1 systemd toolchain-funcs
...
LICENSE="GPL-2"
SLOT="0"
IUSE="cgi +curl cvs doc +gpg highlight +iconv keyring mediawiki +nls +pcre perforce +perl +safe-directory selinux subversion systemd test tk +webdav xinetd"
...
src_configure() {
	...
	if [[ "${PV}" != *9999 ]] ; then
		# Non-live ebuilds download the sources from a tarball which
		# does not include a .git directory.  Coccinelle assumes it
...
src_install() {
	...
	if use perl ; then
		...
		mv "${ED}"/usr/share/perl5/Git.pm \
				"${ED}/$(perl_get_vendorlib)" ||
			die
		mv "${ED}"/usr/share/perl5/Git \
				"${ED}/$(perl_get_vendorlib)" ||
			die
	fi
