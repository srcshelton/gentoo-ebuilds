...
pkg_postinst() {
	...
	if [[ ! -f "${EROOT}/etc/unbound/var/root-anchors.txt" ]]; then
		...
		einfo "set 'auto-trust-anchor-file:" \
			"${EROOT%/}/etc/unbound/var/root-anchors.txt' in" \
				"${EROOT}/etc/unbound/unbound.conf"
		einfo "and run"
		einfo ""
		einfo "  su -s /bin/sh -c" \
			"'${EROOT%/}/usr/sbin/unbound-anchor -a" \
	...
	local _perm_check_testfile="$( #
			mktemp --dry-run \
				"${EPREFIX}"/etc/unbound/var/.pkg_postinst-perm-check.XXXXXXXXX
		)"
	if ! su -s /bin/sh -c "touch ${_perm_check_testfile}" unbound \
			>/dev/null 2>&1
	then
