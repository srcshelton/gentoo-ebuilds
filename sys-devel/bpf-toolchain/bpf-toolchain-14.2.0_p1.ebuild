...
src_compile() {
	...
	# Something in 'gcc-build/gcc/configure' is causing g++ to be passed broken
	# CFLAGS containing '-Wl,-Wl,...'
	# ... which is again going to be a bad substitution where '-Wl,-O1' is
	# mistakenly filtered as an unguarded CC flag rather than a guarded LDFLAG.
	#
	if is-flagq -Wl,-O1; then
		filter-flags -Wl,-O1
