...
IUSE="clang +client lzma multitarget nls +python +server source-highlight test vanilla xml"
REQUIRED_USE="
...
src_prepare() {
	...
	if use clang; then
		epatch "${FILESDIR}"/"${PN}"-8.0.1-clang-configure.patch
		epatch "${FILESDIR}"/"${PN}"-8.0.1-compile-loc2c.c.patch
	fi
	case ${CHOST} in
		*-darwin*)
