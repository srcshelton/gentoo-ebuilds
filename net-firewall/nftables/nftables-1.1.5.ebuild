...
src_configure() {
	local myeconfargs=(
		...
		$(use_with systemd unitdir "$(systemd_get_systemunitdir)")
		$(use_enable debug)
...
src_install() {
	...
	if ! use doc && [[ ! ${PV} =~ ^[9]{4,}$ ]]; then
		...
		newman "${FILESDIR}/man-pages/${PN}-1.1.4-libnftables-json.5" libnftables-json.5
		newman "${FILESDIR}/man-pages/${P}-nft.8" nft.8
		newman "${FILESDIR}/man-pages/${P}-nftables.service.8" nftables.service.8
...
pkg_preinst() {
	...
	if [[ -n "${ROOT}" && "${ROOT}" != '/' ]] || [[ ! -d /sys/module/nftables ]] || [[ ! -x /sbin/nft ]]; then
