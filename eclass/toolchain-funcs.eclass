...
#  - softfp: (arm specific) the target should use hardfloat insns, but
...
tc-ld-is-bfd() {
	...
	if type -pf $(tc-getLD "$@") >/dev/null 2>&1; then
...
_tc-has-openmp() {
	...
	EOF
	$(tc-getCC "$@") -fopenmp "${base}.c" -o "${base}" >/dev/null 2>&1
	local ret=$?
	rm -f "${base}"*
	return ${ret}
}
...
tc-check-openmp() {
	...
	if use openmp; then
		...
		ewarn "but the 'openmp' USE flag is set."
		ewarn "Continuing with installation on the assumption that a compiler"
		ewarn "with OpenMP support will be avaiable by the time the target"
		ewarn "package needs to be built"
		return 0
	fi
...
tc-has-tls() {
	...
	[[ $1 == -* ]] && shift
	$(tc-getCC "$@") ${flags} "${base}.c" -o "${base}" >/dev/null 2>&1
	local ret=$?
	rm -f "${base}"*
	return ${ret}
}
...
# @DESCRIPTION:
# Accepts 'host' as an argument which defaults to CTARGET and falls back to
# CHOST if unspecified.  Returns 'big' or 'little' depending on whether 'host'
...
tc-get-compiler-type() {
	...
	local res="$($(tc-getCC "$@") -E -P - <<<"${code}" 2>/dev/null)"
...
tc-get-ptr-size() {
	$(tc-getCC) -E -P - <<< __SIZEOF_POINTER__ 2>/dev/null ||
...
tc-get-build-ptr-size() {
	$(tc-getBUILD_CC) -E -P - <<< __SIZEOF_POINTER__ 2>/dev/null ||
...
tc-has-64bit-time_t() {
	$(tc-getCC) ${CFLAGS} ${CPPFLAGS} -c -x c - \
			-o /dev/null <<-EOF >/dev/null 2>&1
		#include <sys/types.h>
