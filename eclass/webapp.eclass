...
# WEBBAPP_DEPEND depencency only guarantees that /usr/sbin/webapp-config will
# be available in ROOT='/', not necessarily in ${EROOT}...
#WEBAPP_CONFIG="${EROOT}/usr/sbin/webapp-config"
#WEBAPP_CLEANER="${EROOT}/usr/sbin/webapp-cleaner"
WEBAPP_CONFIG="/usr/sbin/webapp-config"
WEBAPP_CLEANER="/usr/sbin/webapp-cleaner"
...
webapp_read_config() {
	...
	if [[ ! -s "${ETC_CONFIG}" ]] && [[ -s "${ETC_CONFIG#"${EROOT}"}" ]]
	then
		ETC_CONFIG="${ETC_CONFIG#"${EROOT}"}"
	fi

	if has_version '>=app-admin/webapp-config-1.50'; then
		if ENVVAR=$(${WEBAPP_CONFIG} --query ${PN} ${PVR}); then
			eval ${ENVVAR}
		else
			eerror "Could not read settings from webapp-config"
			if [[ -s "${ETC_CONFIG}" ]]; then
				ewarn "Falling-back to reading file '${ETC_CONFIG}' ..."
				. ${ETC_CONFIG} || die "Unable to process '${ETC_CONFIG}'"
			else
				die "Could not read settings from webapp-config or '${ETC_CONFIG}'"
			fi
		fi
