...
LICENSE="GPL-2"
SLOT="0"
IUSE="caps junction kerberos ldap +libmount nfsdctl +nfsv3 +nfsv4 sasl selinux systemd tcpd udev +uuid"
...
src_prepare() {
	...
	sed \
		...
		-i \
			...
			tests/test-lib.sh \
			systemd/*.service
	if (( grep -R '/run/' . | grep -v '/var/run/' | wc -l )); then
		local line=''

		eerror "Found references to '/run/' in files:"
		grep -HR '/run/' . | grep -v '/var/run/' | while read -r line; do
			error "    ${line}"
		done
		die "Source references '/run/'"
	fi
...
src_install() {
	...
	if [[ -x "${ED}"/sbin/nfsdcltrack ]]; then
		# nfsdcltrack seems to be being built regardless of the 'nfsv4'
		# USE flag...
		#
