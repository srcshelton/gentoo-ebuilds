...
src_prepare() {
	...
	eapply -p2 "${FILESDIR}/${P}-update-ca-certificates-drop-pointless-dependency.patch"
...
src_install() {
	...
	use update || rm "${D}"/usr/sbin/update-ca-certificates
...
pkg_postinst() {
	if [[ -d "${EROOT}/usr/local/share/ca-certificates" ]] ; then
		if ! use update; then
			ewarn "Certificates in ${EROOT}/usr/local/share/ca-certificates" \
				"will not be included in the db when built without" \
				"USE='update'"
		else
