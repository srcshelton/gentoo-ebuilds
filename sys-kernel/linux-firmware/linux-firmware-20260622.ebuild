...
_normalise_config() {
	...
	sed -i -e 's/\.xz$//' -e 's/\.zst$//' "${file}" ||
		die "Failed to remove compression suffixes from '${file}'"
}
...
pkg_setup() {
	if use compress-xz || use compress-zstd ; then
		...
		if has_version sys-kernel/raspberrypi-image; then
			eerror "sys-kernel/raspberrypi-image kernels cannot load compressed firmware"
		fi
	fi
}
...
src_prepare() {
	...
	if use !redistributable; then
		# remove files _not_ in the free_software lists everything else
		# is confirmed (or assumed) to be redistributable based on
		# upstream acceptance policy
...
src_install() {
	...
	if [[ -n "$( find * -xtype l -print )" ]]; then
		die "Failed to delete broken symlinks"
	fi
	...
	if ! use savedconfig; then
		...
		#restore_config "${S}/${PN}.conf"

		if ! [[ -r "${S}/${PN}.conf" ]]; then
			eerror "Unable to restore saved configuration to '${S}/${PN}.conf' - keeping all firmware blobs"
			mv "${S}/${PN}.conf.dist" "${S}/${PN}.conf"
		else
			rm "${S}/${PN}.conf.dist"
		fi

		echo ; einfo "Files to be kept:"
		local line dir
		local -a dirs=()
		while read -r line; do
			if [[ "${line}" == */* ]]; then
				dir="${line/\/*}"
				if ! grep -qw "${dir}" <<<"${dirs[*]}"; then
					dirs+=( "${dir}" )
				fi
			fi
			einfo "    ${line}"
		done < <( sed 's/#.*$//' "${S}/${PN}.conf" | grep -v '^\s*$' )
		echo ; einfo "Directories to be kept:"
		for dir in "${dirs[@]}"; do
			einfo "    ${dir}"
		done

		local -i p=0
		find * \( \! -type d -and \! -name "${S}/${PN}.conf"\* \)	\
				| sort "${S}/${PN}.conf" -							\
				| sed 's/#.*$//' | grep -v '^\s*$'				\
				| uniq -u | while read -r line; do
			dir="${line/\/*}"
			if grep -qw "${dir}" <<<"${dirs[*]}"; then
				if ! (( p )); then
					echo
				...
				ewarn "Removing file '${line}' from saved directory '${dir}'"
			fi
		done
		...
		while read -r line; do
			if [[ -n "${line}" ]]; then
				if ! [[ -r "${line}" ]]; then
					...
					ewarn "File '${line}' in saved config no longer exists"
				fi
			fi
		done < <( sed 's/#.*$//' "${S}/${PN}.conf" | grep -v '^\s*$' )
	...
	if use initramfs; then
		# Instruct Dracut on whether or not we want the microcode in
		# initramfs
