...
KEYWORDS="arm arm64"
IUSE="+acpi-table-upgrade +acpi-table-upgrade-dsdt +acpi-table-upgrade-iort-httu +acpi-table-upgrade-iort-msi experimental npu-r2p2-abi +radxa-menu"
REQUIRED_USE="
...
src_prepare() {
	...
	# CIX's native fwnode regulator support still needs these ACPI PRP0001
	# fixed-regulator corrections for Orion firmware-described board rails.
	eapply "${FILESDIR}"/7.1.x/50096-regulator-fixed-match-acpi-prp0001-without-of.patch || die
	eapply "${FILESDIR}"/7.1.x/50097-regulator-fixed-keep-acpi-fixed-rails-enabled.patch || die

	eapply "${FILESDIR}"/7.1.x/0046-cix-tty-amba-pl011-use-driver-from-cix-bsp.patch || die
	eapply "${FILESDIR}"/7.1.x/20071-tty-amba-pl011-drop-stale-console-line-ended-write.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/20020-phy-cix-fix-dependencies-and-clang-uninitialised-use.patch || die
	eapply "${FILESDIR}"/7.0.x/20025-asoc-cix-card-utils-fix-clang-uninitialised-use.patch || die
	eapply "${FILESDIR}"/7.0.x/30000-pmdomain-fix-acpi-scmi-perf-domain-wiring.patch || die
	eapply "${FILESDIR}"/7.0.x/30015-pmdomain-export-genpd-dev-pm-attach-by-name.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/71025-cix-mvx-fix-vb2-queue-setup-on-linux-7.patch || die
	...
	eapply "${FILESDIR}"/7.1.x/71090-cix-mvx-uplift-p1-7.0-v1.0.2-core-fixes.patch || die
	eapply "${FILESDIR}"/7.1.x/71100-cix-mvx-uplift-p1-7.0-v1.0.2-session-api.patch || die
	eapply "${FILESDIR}"/7.1.x/71110-cix-mvx-fix-w1-warnings.patch || die
	eapply "${FILESDIR}"/7.1.x/71111-cix-mvx-annotate-log-format-functions.patch || die
	eapply "${FILESDIR}"/7.1.x/71112-cix-mvx-fix-kernel-doc-warnings.patch || die
	...
	eapply "${FILESDIR}"/7.1.x/72100-media-cix-armcb-isp-uplift-2026q2-release.patch || die
	...
	eapply "${FILESDIR}"/7.1.x/70020-drm-cix-gate-virtual-encoder-build.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/40050-soc-cix-arbitrate-acpi-usb-models.patch || die
	eapply "${FILESDIR}"/7.0.x/40060-soc-cix-add-gpu-cca-scan-quirk.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/90000-soc-cix-add-acpi-bus-perf-driver.patch || die
	eapply "${FILESDIR}"/7.0.x/71065-cix-mvx-guard-optional-devfreq-path.patch || die
	eapply "${FILESDIR}"/7.0.x/71988-armchina-npu-guard-optional-dma-buf-path.patch || die
	eapply "${FILESDIR}"/7.0.x/90010-cix-sky1-add-acpi-platform-services.patch || die
	eapply "${FILESDIR}"/7.0.x/90030-thermal-cix-cpu-ipa-support-acpi-resources.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/60015-usb-cdnsp-sky1-tear-down-host-on-shutdown.patch || die
	...
	eapply "${FILESDIR}"/7.1.x/30170-clk-sky1-acpi-select-cix-mailbox-for-scmi.patch || die
	...
	eapply "${FILESDIR}"/7.1.x/50022-irqchip-sky1-pdc-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/72107-media-cix-armcb-isp-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/80062-realtek-r8126-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/60097-phy-cix-usbdp-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/40054-pinctrl-sky1-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/30022-pmdomain-core-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/20072-tty-amba-pl011-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/73040-asoc-cix-fix-gcc15-clang21-w1-findings.patch || die
	eapply "${FILESDIR}"/7.1.x/70130-drm-cix-retain-downstream-display-improvements.patch || die
	eapply "${FILESDIR}"/7.1.x/50100-i2c-cadence-retain-downstream-acpi-improvements.patch || die
	eapply "${FILESDIR}"/7.1.x/50110-i3c-cadence-retain-downstream-acpi-improvements.patch || die
	eapply "${FILESDIR}"/7.1.x/50120-pwm-sky1-retain-downstream-acpi-improvements.patch || die
	eapply "${FILESDIR}"/7.1.x/50130-spi-cadence-retain-downstream-acpi-improvements.patch || die
	eapply "${FILESDIR}"/7.1.x/50095-regulator-fwnode-retain-downstream-improvements.patch || die
...
pkg_postinst() {
	...
	elog
	if use npu-r2p2-abi; then
		elog "The opt-in ArmChina NPU R2P2 kernel ABI is enabled."
		elog "Use only with an R2P2-compatible userspace runtime."
	else
		elog "The ArmChina NPU driver uses the R2P0 ABI expected by"
		elog "cix-noe-umd 2.0.2 and libnoe.so.0.6.0."
	fi
