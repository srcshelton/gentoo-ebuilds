...
CIX_CORE_PATCHES=(
	"${FILESDIR}"/7.0.x/10000-arm64-stub-fdt.patch
	"${FILESDIR}"/10010-arm64-stub-fdt-enable-kexec-file.patch
	"${FILESDIR}"/10020-lld-timer-of-table-end-warning.patch
	"${FILESDIR}"/7.0.x/10040-bpf-guard-session-return-btf-id.patch
	"${FILESDIR}"/7.0.x/10050-bpf-gate-struct-ops-on-kallsyms.patch
)
...
src_prepare() {
	...
	cix_apply_patch() {
		...
		case "${pf##*/}" in
			0001-mailbox-add-acpi-support-to-cix-mailbox-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0001-mailbox-cix-add-audited-acpi-support.patch || die
				return
				;;
			...
			0003-firmware-arm_scmi-add-acpi-support-to-SCMI.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0003-firmware-arm-scmi-add-audited-acpi-support.patch || die
				return
				;;
			0002-acpi-Add-a-property-reference-count-interface.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0002-acpi-cix-resolve-legacy-graph-references.patch || die
				return
				;;
			0004-clk-clk-scmi-register-clkdev-for-acpi.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0004-clk-scmi-add-audited-acpi-publication.patch || die
				return
				;;
			0005-clk-add-cix-clk-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0005-clk-cix-add-audited-sky1-support.patch || die
				return
				;;
			0006-reset-add-cix-reset-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0006-reset-cix-add-audited-sky1-support.patch || die
				return
				;;
			0007-soc-add-cix-acpi-resource-lookup-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0007-soc-cix-harden-acpi-resource-lookup-driver.patch || die
				return
				;;
			0008-pmdomain-add-acpi-support-to-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0008-pmdomain-add-audited-acpi-scmi-support.patch || die
				return
				;;
			...
			0018-clk-clkdev-increase-clkdev-MAX_CON_ID-from-16-to-32.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0018-clk-clkdev-increase-clkdev-MAX_CON_ID-from-16-to-32.patch || die
				return
				;;
			0025-usb-add-usb-cdns3-driver-for-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0025-usb-cdns3-add-audited-sky1-platform-support.patch || die
				return
				;;
			0026-typec-add-rts5453-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0026-usb-typec-rts5453-add-audited-driver.patch || die
				return
				;;
			0027-soc-add-cix-acpi-usb-scan-handler.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0027-soc-cix-arbitrate-acpi-usb-models.patch || die
				return
				;;
			0029-usb-cdns3-fix-spin-lock-issue-in-suspend-resume.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0029-usb-cdns3-propagate-role-pm-errors.patch || die
				return
				;;
			...
			0013-DPTSW-26459-sound-hda-rework-cix-ipbloq-based-on-mai.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0013-sound-hda-cix-add-audited-sky1-support.patch || die
				return
				;;
			...
			0009-remoteproc-add-cix-dsp-remoteproc-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0009-remoteproc-cix-sky1-add-audited-hifi5-support.patch || die
				return
				;;
			...
			0016-dma-arm-dma350-add-acpi-support-for-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0016-dma-arm-dma350-add-audited-cix-support.patch || die
				return
				;;
			0017-gpio-add-acpi-support-to-cadence-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0017-gpio-cadence-add-audited-cix-sky1-support.patch || die
				return
				;;
			0019-i2c-add-acpi-support-for-cadence-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0019-i2c-cadence-add-audited-acpi-support.patch || die
				return
				;;
			0044-pwm-sky1-check-pwm-state-before-enable-disable-clk-i.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0044-pwm-sky1-harden-lifecycle-and-state-validation.patch || die
				return
				;;
			0067-PROJ031-2-backlight-pwm-backlight-add-acpi-support-f.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0067-backlight-pwm-add-safe-firmware-node-support.patch || die
				return
				;;
			0011-drm-panthor-add-acpi-support-for-cix-p1.patch)
				# Replace the vendor import at its stable logical position.
				eapply "${FILESDIR}"/7.0.x/0011-drm-panthor-add-sky1-acpi-support.patch || die
				return
				;;
			...
			0012-irqchip-add-cix-sky1-pdc-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0012-irqchip-cix-sky1-pdc-add-audited-wake-domain.patch || die
				return
				;;
			0024-phy-add-cix-phy-driver.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0024-phy-cix-add-audited-usbdp-combo-phy.patch || die
				return
				;;
			...
			0048-pinctrl-sky1-add-acpi-support.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0048-pinctrl-sky1-add-audited-acpi-support.patch || die
				return
				;;
			0060-ACPI-thermal-bind-devfreq-cooling-devices-via-devfre.patch)
				...
				eapply "${FILESDIR}"/7.0.x/0060-acpi-thermal-bind-devfreq-cooling-devices-safely.patch || die
				return
				;;
	...
	post_vendor_patches=(
		...
		"${FILESDIR}"/7.0.x/20065-cacheinfo-share-global-firmware-ids-across-levels.patch
		...
		"${FILESDIR}"/7.0.x/30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch
		"${FILESDIR}"/7.0.x/30128-acpi-thermal-retain-downstream-improvements.patch
		"${FILESDIR}"/7.0.x/30129-thermal-cix-add-safe-ipa-support.patch
		...
		"${FILESDIR}"/7.0.x/40042-platform-acpi-resolve-named-irq-resources.patch
		# CIX-QUESTIONED-PATCH: stock-firmware log demotion; upgraded DSDTs remove the duplicate.
		"${FILESDIR}"/7.0.x/40046-acpi-demote-cix-sky1-ecam-duplicate-reservations.patch
		"${FILESDIR}"/7.0.x/40052-pinctrl-sky1-validate-o6-camera-mclk-duplication.patch
		"${FILESDIR}"/40056-soc-cix-add-safe-bus-performance-domain-driver.patch
		"${FILESDIR}"/7.0.x/40070-soc-cix-arbitrate-acpi-pcie-models.patch
		...
		"${FILESDIR}"/7.0.x/50130-spi-cadence-add-audited-cix-acpi-support.patch
		"${FILESDIR}"/7.0.x/70020-drm-cix-gate-virtual-encoder-build.patch
		...
		"${FILESDIR}"/7.0.x/70130-drm-cix-retain-safe-display-improvements.patch
		"${FILESDIR}"/7.0.x/70135-drm-cix-remove-unsafe-engineering-interfaces.patch
		"${FILESDIR}"/7.0.x/70150-drm-support-up-to-64-planes.patch
		"${FILESDIR}"/70160-drm-cix-dptx-fix-audio-eld-and-shutdown.patch
		"${FILESDIR}"/7.0.x/70200-drm-panthor-declare-scmi-perf-softdep.patch
		"${FILESDIR}"/7.0.x/70990-media-cix-import-and-integrate-mvx-vpu-driver.patch
		...
		"${FILESDIR}"/7.0.x/71110-cix-mvx-fix-source-quality.patch
		"${FILESDIR}"/7.0.x/71120-cix-mvx-harden-dma-firmware-lifetime-and-devfreq.patch
		"${FILESDIR}"/7.0.x/71130-cix-mvx-remove-unsafe-kernel-buffer-dump.patch
		"${FILESDIR}"/7.0.x/71150-cix-mvx-fix-single-planar-dmabuf-capture.patch
		"${FILESDIR}"/7.0.x/71160-cix-mvx-harden-fault-and-lifecycle-recovery.patch
		"${FILESDIR}"/7.0.x/71500-misc-armchina-npu-import-sky1-driver.patch
		"${FILESDIR}"/7.0.x/71510-misc-armchina-npu-harden-raw-register-io.patch
		"${FILESDIR}"/71520-misc-armchina-npu-restrict-to-cix-sky1-v3.patch
		"${FILESDIR}"/7.0.x/71530-misc-armchina-npu-harden-ownership-and-domains.patch
		"${FILESDIR}"/7.0.x/71540-misc-armchina-npu-add-scmi-opp-devfreq.patch
		"${FILESDIR}"/7.0.x/71550-misc-armchina-npu-balance-runtime-pm.patch
		"${FILESDIR}"/7.0.x/71560-misc-armchina-npu-harden-userspace-abi-and-dma.patch
		"${FILESDIR}"/7.0.x/71570-misc-armchina-npu-harden-fault-teardown-and-source-quality.patch
		"${FILESDIR}"/7.0.x/71580-misc-armchina-npu-link-devfreq-providers.patch
		...
		"${FILESDIR}"/7.0.x/72000-media-cix-import-armcb-isp-driver.patch
		"${FILESDIR}"/7.0.x/72010-media-cix-harden-armcb-isp-platform-subdevices.patch
		"${FILESDIR}"/7.0.x/72015-media-cix-harden-armcb-isp-v4l2-streaming.patch
		"${FILESDIR}"/7.0.x/72020-media-cix-harden-armcb-isp-dma-legacy-abi.patch
		"${FILESDIR}"/7.0.x/72025-media-cix-coordinate-ddr-low-power-with-isp.patch
		"${FILESDIR}"/7.0.x/73050-sound-soc-cix-harden-audio-paths.patch
		...
		"${FILESDIR}"/7.0.x/80015-bluetooth-btrtl-return-register-read-error.patch
		"${FILESDIR}"/7.0.x/80020-rtw89-check-acpi-dsm-before-evaluating.patch
		"${FILESDIR}"/7.0.x/80025-cadence-macb-add-sky1-firmware-matches.patch
		"${FILESDIR}"/7.0.x/80030-net-realtek-import-r8126-driver.patch
		...
		"${FILESDIR}"/7.0.x/80070-pci-disable-aspm-for-sky1-smmu-faulting-endpoints.patch
		"${FILESDIR}"/7.0.x/90040-hwmon-cix-add-safe-acpi-fan-control.patch
	)
	...
	if use radxa-menu; then
		eapply "${FILESDIR}"/7.0.x/90050-arm64-cix-add-radxa-orion-board-profiles.patch || die
	fi
	eapply \
