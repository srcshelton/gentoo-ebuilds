...
PATCHES=(
	"${FILESDIR}"/7.0.x/cix-3aad824/10000-arm64-stub-fdt.patch
...
src_prepare() {
	...
	eapply "${FILESDIR}"/cix-3aad824/70010-drm-cix-dptx-fix-clang-werror-in-component-bypass-builds.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/30000-cix-3aad824-pmdomain-fix-acpi-scmi-perf-domain-wiring.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/30015-cix-3aad824-pmdomain-export-genpd-dev-pm-attach-by-name.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/70020-cix-display-and-backlight-build-fixes.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/70050-drm-cix-enable-acpi-stub-fdt-display.patch || die
	eapply "${FILESDIR}"/cix-3aad824/70060-drm-add-fwnode-panel-bridge-helpers.patch || die
	eapply "${FILESDIR}"/cix-3aad824/70070-drm-cix-use-fwnode-display-links.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/70100-cix-3aad824-drm-cix-linlon-dp-fix-clang-warnings.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/70110-cix-3aad824-drm-cix-demote-display-info-logs.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/40050-cix-3aad824-soc-cix-arbitrate-acpi-usb-models.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/40060-cix-3aad824-soc-cix-add-gpu-cca-scan-quirk.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/40070-cix-3aad824-soc-cix-arbitrate-acpi-pcie-models.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/90000-cix-3aad824-soc-cix-add-acpi-bus-perf-driver.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/90010-cix-3aad824-cix-sky1-acpi-socinfo-nvmem-ddrlp-ipa.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/90020-cix-3aad824-cix-fix-module-modpost-exports.patch || die
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/90030-cix-cpu-ipa-use-dtb-register-size.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/90096-soc-cix-add-sky1-reboot-reason-driver.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/cix-3aad824/30110-cix-acpi-ids-and-clkt-consumer-fixes.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/cix-759efc0/73000-cix-hda-require-cadence-gpio-on-acpi-systems.patch || die
