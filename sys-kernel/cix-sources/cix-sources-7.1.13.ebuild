...
CIX_CORE_PATCHES=(
	...
	"${FILESDIR}"/7.1.x/10050-bpf-gate-struct-ops-on-kallsyms.patch
...
src_prepare() {
	...
	cix_apply_patch() {
		...
		case "${pf##*/}" in
			0001-mailbox-add-acpi-support-to-cix-mailbox-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0001-mailbox-cix-add-audited-acpi-support.patch || die
				return
				;;
			0003-firmware-arm_scmi-add-acpi-support-to-SCMI.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0003-firmware-arm-scmi-add-audited-acpi-support.patch || die
				return
				;;
			0002-acpi-Add-a-property-reference-count-interface.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0002-acpi-cix-resolve-legacy-graph-references.patch || die
				return
				;;
			0004-clk-clk-scmi-register-clkdev-for-acpi.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0004-clk-scmi-add-audited-acpi-publication.patch || die
				return
				;;
			00050-dt-bindings-soc-cix-add-sky1-audss-cru-controller.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0005-clk-cix-add-audited-sky1-support.patch || die
				return
				;;
			...
			00060-reset-cix-add-sky1-audss-auxiliary-reset-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0006-reset-cix-add-audited-sky1-support.patch || die
				return
				;;
			...
			0007-soc-add-cix-acpi-resource-lookup-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0007-soc-cix-harden-acpi-resource-lookup-driver.patch || die
				return
				;;
			0008-pmdomain-add-acpi-support-to-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0008-pmdomain-add-audited-acpi-scmi-support.patch || die
				return
				;;
			...
			0023-syscon-add-device_syscon_regmap_lookup_by_property.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0023-sound-sof-clean-up-debugfs-lifetime.patch || die
				return
				;;
			0018-clk-clkdev-increase-clkdev-MAX_CON_ID-from-16-to-32.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0018-clk-clkdev-increase-clkdev-MAX_CON_ID-from-16-to-32.patch || die
				return
				;;
			0025-usb-add-usb-cdns3-driver-for-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0025-usb-cdns3-add-audited-sky1-platform-support.patch || die
				return
				;;
			0026-typec-add-rts5453-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0026-usb-typec-rts5453-add-audited-driver.patch || die
				return
				;;
			0027-soc-add-cix-acpi-usb-scan-handler.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0027-soc-cix-arbitrate-acpi-usb-models.patch || die
				return
				;;
			0029-usb-cdns3-fix-spin-lock-issue-in-suspend-resume.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0029-usb-cdns3-propagate-role-pm-errors.patch || die
				return
				;;
			...
			0013-DPTSW-26459-sound-hda-rework-cix-ipbloq-based-on-mai.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0013-sound-hda-cix-add-audited-sky1-support.patch || die
				return
				;;
			...
			0009-remoteproc-add-cix-dsp-remoteproc-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0009-remoteproc-cix-sky1-add-audited-hifi5-support.patch || die
				return
				;;
			...
			0021-sound-soc-add-cix-sof-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0021-sound-soc-cix-sky1-add-audited-sof-support.patch || die
				return
				;;
			0016-dma-arm-dma350-add-acpi-support-for-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0016-dma-arm-dma350-add-audited-cix-support.patch || die
				return
				;;
			0017-gpio-add-acpi-support-to-cadence-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0017-gpio-cadence-add-audited-cix-sky1-support.patch || die
				return
				;;
			0019-i2c-add-acpi-support-for-cadence-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0019-i2c-cadence-add-audited-acpi-support.patch || die
				return
				;;
			0044-pwm-sky1-check-pwm-state-before-enable-disable-clk-i.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0044-pwm-sky1-harden-lifecycle-and-state-validation.patch || die
				return
				;;
			0067-PROJ031-2-backlight-pwm-backlight-add-acpi-support-f.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0067-backlight-pwm-add-safe-firmware-node-support.patch || die
				return
				;;
			00111-drm-panthor-add-ACPI-CIXH5000-platform-binding.patch)
				# Replace the vendor import at its stable logical position.
				eapply "${FILESDIR}"/7.1.x/0011-drm-panthor-add-sky1-acpi-support.patch || die
				return
				;;
			...
			0012-irqchip-add-cix-sky1-pdc-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0012-irqchip-cix-sky1-pdc-add-audited-wake-domain.patch || die
				return
				;;
			0024-phy-add-cix-phy-driver.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0024-phy-cix-add-audited-usbdp-combo-phy.patch || die
				return
				;;
			...
			0048-pinctrl-sky1-add-acpi-support.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0048-pinctrl-sky1-add-audited-acpi-support.patch || die
				return
				;;
			0060-ACPI-thermal-bind-devfreq-cooling-devices-via-devfre.patch)
				...
				eapply "${FILESDIR}"/7.1.x/0060-acpi-thermal-bind-devfreq-cooling-devices-safely.patch || die
				return
				;;
	...
	post_vendor_patches=(
		...
		"${FILESDIR}"/7.1.x/20065-cacheinfo-share-global-firmware-ids-across-levels.patch
		"${FILESDIR}"/7.1.x/20070-resctrl-mpam-expose-proportional-bandwidth.patch
		...
		"${FILESDIR}"/7.1.x/30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch
		"${FILESDIR}"/7.1.x/30128-acpi-thermal-retain-downstream-improvements.patch
		"${FILESDIR}"/7.1.x/30129-thermal-cix-add-safe-ipa-support.patch
		...
		"${FILESDIR}"/7.1.x/40042-platform-acpi-resolve-named-irq-resources.patch
		# CIX-QUESTIONED-PATCH: stock-firmware log demotion; upgraded DSDTs remove the duplicate.
		"${FILESDIR}"/7.1.x/40046-acpi-demote-cix-sky1-ecam-duplicate-reservations.patch
		"${FILESDIR}"/7.1.x/40052-pinctrl-sky1-validate-o6-camera-mclk-duplication.patch
		...
		"${FILESDIR}"/7.1.x/40070-soc-cix-arbitrate-acpi-pcie-models.patch
		"${FILESDIR}"/40093-pci-cix-enable-root-port-io-window-assignment.patch
		"${FILESDIR}"/7.1.x/50030-net-bridge-warn-for-missing-netfilter-on-first-device.patch
		"${FILESDIR}"/7.1.x/50060-watchdog-sbsa-gwdt-use-cix-sky1-refresh-value.patch
		"${FILESDIR}"/7.1.x/50130-spi-cadence-add-audited-cix-acpi-support.patch
		"${FILESDIR}"/7.1.x/60010-usb-cdns3-sky1-declare-phy-dependency.patch
		"${FILESDIR}"/7.1.x/70020-drm-cix-gate-virtual-encoder-build.patch
		...
		"${FILESDIR}"/7.1.x/70130-drm-cix-retain-safe-display-improvements.patch
		"${FILESDIR}"/7.1.x/70135-drm-cix-remove-unsafe-engineering-interfaces.patch
		"${FILESDIR}"/7.1.x/70140-drm-cix-fix-gcc15-clang21-w1-findings.patch
		"${FILESDIR}"/7.1.x/70150-drm-support-up-to-64-planes.patch
		...
		"${FILESDIR}"/7.1.x/70200-drm-panthor-declare-scmi-perf-softdep.patch
		"${FILESDIR}"/7.1.x/70990-media-cix-import-and-integrate-mvx-vpu-driver.patch
		...
		"${FILESDIR}"/7.1.x/71080-cix-mvx-uplift-selected-2026q2-fixes.patch
		"${FILESDIR}"/7.1.x/71090-cix-mvx-uplift-p1-7.0-v1.0.2-core-fixes.patch
		"${FILESDIR}"/7.1.x/71100-cix-mvx-uplift-p1-7.0-v1.0.2-session-api.patch
		"${FILESDIR}"/7.1.x/71110-cix-mvx-fix-source-quality.patch
		"${FILESDIR}"/7.1.x/71120-cix-mvx-harden-dma-firmware-lifetime-and-devfreq.patch
		"${FILESDIR}"/7.1.x/71130-cix-mvx-remove-unsafe-kernel-buffer-dump.patch
		# CIX-QUESTIONED-PATCH: disabled; see the patch preamble.
		# "${FILESDIR}"/7.1.x/71140-cix-mvx-add-rrc-dqp-session-controls.patch
		"${FILESDIR}"/7.1.x/71150-cix-mvx-fix-single-planar-dmabuf-capture.patch
		"${FILESDIR}"/7.1.x/71160-cix-mvx-harden-fault-and-lifecycle-recovery.patch
		"${FILESDIR}"/7.1.x/71500-misc-armchina-npu-import-sky1-driver.patch
		"${FILESDIR}"/7.1.x/71510-misc-armchina-npu-harden-raw-register-io.patch
		"${FILESDIR}"/71520-misc-armchina-npu-restrict-to-cix-sky1-v3.patch
		"${FILESDIR}"/7.1.x/71530-misc-armchina-npu-harden-ownership-and-domains.patch
		"${FILESDIR}"/7.1.x/71540-misc-armchina-npu-add-scmi-opp-devfreq.patch
		"${FILESDIR}"/7.1.x/71550-misc-armchina-npu-balance-runtime-pm.patch
		"${FILESDIR}"/7.1.x/71560-misc-armchina-npu-harden-userspace-abi-and-dma.patch
		"${FILESDIR}"/7.1.x/71570-misc-armchina-npu-harden-fault-teardown-and-source-quality.patch
		"${FILESDIR}"/7.1.x/71580-misc-armchina-npu-link-devfreq-providers.patch
		...
		"${FILESDIR}"/7.1.x/72000-media-cix-import-armcb-isp-driver.patch
		"${FILESDIR}"/7.1.x/72010-media-cix-harden-armcb-isp-platform-subdevices.patch
		"${FILESDIR}"/7.1.x/72015-media-cix-harden-armcb-isp-v4l2-streaming.patch
		"${FILESDIR}"/7.1.x/72020-media-cix-harden-armcb-isp-dma-legacy-abi.patch
		"${FILESDIR}"/7.1.x/72025-media-cix-coordinate-ddr-low-power-with-isp.patch
		"${FILESDIR}"/7.1.x/73050-sound-soc-cix-harden-audio-paths.patch
		"${FILESDIR}"/7.1.x/73060-sound-cix-declare-runtime-provider-dependencies.patch
		...
		"${FILESDIR}"/7.1.x/80015-bluetooth-btrtl-return-register-read-error.patch
		"${FILESDIR}"/7.1.x/80020-rtw89-check-acpi-dsm-before-evaluating.patch
		"${FILESDIR}"/7.1.x/80025-cadence-macb-add-sky1-firmware-matches.patch
		"${FILESDIR}"/7.1.x/80030-net-realtek-import-r8126-driver.patch
		...
		"${FILESDIR}"/7.1.x/80070-pci-disable-aspm-for-sky1-smmu-faulting-endpoints.patch
		"${FILESDIR}"/7.1.x/90040-hwmon-cix-add-safe-acpi-fan-control.patch
		"${FILESDIR}"/7.1.x/90045-cix-close-platform-driver-dependencies.patch
	)
	...
	if use radxa-menu; then
		...
		eapply "${FILESDIR}"/7.1.x/90060-arm64-cix-model-orion-acpi-infrastructure.patch || die
	fi
	eapply \
		"${FILESDIR}"/90096-soc-cix-add-firmware-scratch-diagnostics.patch \
		"${FILESDIR}"/7.1.x/90098-pstore-ramoops-parse-firmware-node-properties.patch || die
...
pkg_postinst() {
	...
	if use acpi-table-upgrade; then
		...
		if use acpi-table-upgrade-dsdt; then
			...
			elog "is qualified."
			elog "The Linux 7.1 O6 and O6N firmware-1.2 DSDT profiles also"
