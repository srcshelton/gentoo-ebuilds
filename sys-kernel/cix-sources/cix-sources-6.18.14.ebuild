...
src_prepare() {
	...
	sky1_apply_series_patch() {
		...
		case "${pf}" in
			0065-treewide-Add-ACPI-device-IDs-for-CIX-Sky1-SoC-periph.patch)
				eapply "${FILESDIR}"/cix-3aad824/6.18.x/40005-add-remaining-sky1-acpi-device-ids.patch || die
				return
				;;
		esac
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/71995-armchina-npu-restore-local-acpi-dma-lifetime-fixes.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/20011-cix-fix-deps-section-mismatch-and-clang-uninit-build-fail.patch || die
	eapply "${FILESDIR}"/6.18.x/90070-sky1-restore-cadence-torrent-dt-binding-header.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/70005-drm-cix-linlon-dp-fix-symbol-clashes-and-clang-werror.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/73000-cix-hda-require-cadence-gpio-on-acpi-systems.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/73010-cix-hda-prefer-acpi-dma-ranges-and-harden-probe.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/70060-drm-add-fwnode-panel-bridge-helpers.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/30020-pmdomain-fix-acpi-scmi-perf-domain-wiring.patch || die
	eapply "${FILESDIR}"/6.18.x/30160-scmi-handle-acpi-debugfs-fallbacks.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/50050-sky1-acpi-runtime-driver-fixes.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/60040-usb-typec-acpi-runtime-fixes.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/60096-phy-cix-usbdp-allow-acpi-selection.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/70040-display-media-acpi-runtime-fixes.patch || die
	eapply "${FILESDIR}"/6.18.x/80020-rtw89-check-acpi-dsm-before-evaluating.patch || die
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/90000-soc-cix-add-acpi-runtime-drivers.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/6.18.x/80081-cix-sky1-declare-module-softdeps.patch || die
