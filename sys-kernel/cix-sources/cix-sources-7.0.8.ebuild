...
PATCHES=(
	"${FILESDIR}"/cix-3aad824/7.0.x/10000-arm64-stub-fdt.patch
...
src_prepare() {
	...
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/50000-iommu-arm-smmu-v3-add-acpi-boot-active-bypass-stes-for-cix-sky1-pcie.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/20010-cix-fix-deps-section-mismatch-and-clang-uninit-build-fail.patch || die
	eapply "${FILESDIR}"/cix-3aad824/shared/70010-drm-cix-dptx-fix-clang-werror-in-component-bypass-builds.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/30000-cix-3aad824-pmdomain-fix-acpi-scmi-perf-domain-wiring.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/30015-cix-3aad824-pmdomain-export-genpd-dev-pm-attach-by-name.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/shared/30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/shared/70020-cix-display-and-backlight-build-fixes.patch || die
	eapply "${FILESDIR}"/70030-drm-cix-dptx-make-extra-stream-clocks-optional.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/70040-drm-panthor-drop-unused-gem-device-variable.patch || die
	eapply "${FILESDIR}"/cix-3aad824/shared/70050-drm-cix-enable-acpi-stub-fdt-display.patch || die
	eapply "${FILESDIR}"/cix-3aad824/shared/70060-drm-add-fwnode-panel-bridge-helpers.patch || die
	eapply "${FILESDIR}"/cix-3aad824/shared/70070-drm-cix-use-fwnode-display-links.patch || die
	eapply "${FILESDIR}"/70080-drm-cix-remove-unused-dptx-cadence-phy-kconfig.patch || die
	eapply "${FILESDIR}"/cix-3aad824/shared/70090-drm-cix-remove-unused-display-kconfig-prompts.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/70100-cix-3aad824-drm-cix-linlon-dp-fix-clang-warnings.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/70110-cix-3aad824-drm-cix-demote-display-info-logs.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/40050-cix-3aad824-soc-cix-arbitrate-acpi-usb-models.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/40060-cix-3aad824-soc-cix-add-gpu-cca-scan-quirk.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/40070-cix-3aad824-soc-cix-arbitrate-acpi-pcie-models.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/90000-cix-3aad824-soc-cix-add-acpi-bus-perf-driver.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/90010-cix-3aad824-cix-sky1-acpi-socinfo-nvmem-ddrlp-ipa.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/90020-cix-3aad824-cix-fix-module-modpost-exports.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/90030-cix-cpu-ipa-use-dtb-register-size.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/shared/90096-soc-cix-add-sky1-reboot-reason-driver.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/90098-pstore-ramoops-parse-firmware-node-properties.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/30110-cix-acpi-ids-and-clkt-consumer-fixes.patch || die
	...
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/73000-cix-hda-require-cadence-gpio-on-acpi-systems.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/73010-cix-hda-prefer-acpi-dma-ranges-and-harden-probe.patch || die
	eapply "${FILESDIR}"/cix-3aad824/7.0.x/50010-gpio-cadence-restore-match-data-and-skip-init.patch || die
