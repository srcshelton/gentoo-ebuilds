...
src_prepare() {
	...
	eapply "${FILESDIR}"/7.0.x/30000-cix-3aad824-pmdomain-fix-acpi-scmi-perf-domain-wiring.patch || die
	eapply "${FILESDIR}"/7.0.x/30015-cix-3aad824-pmdomain-export-genpd-dev-pm-attach-by-name.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/70100-cix-3aad824-drm-cix-linlon-dp-fix-clang-warnings.patch || die
	eapply "${FILESDIR}"/7.0.x/70110-cix-3aad824-drm-cix-demote-display-info-logs.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/40050-cix-3aad824-soc-cix-arbitrate-acpi-usb-models.patch || die
	eapply "${FILESDIR}"/7.0.x/40060-cix-3aad824-soc-cix-add-gpu-cca-scan-quirk.patch || die
	eapply "${FILESDIR}"/7.0.x/40070-cix-3aad824-soc-cix-arbitrate-acpi-pcie-models.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/90000-cix-3aad824-soc-cix-add-acpi-bus-perf-driver.patch || die
	eapply "${FILESDIR}"/7.0.x/90010-cix-3aad824-cix-sky1-acpi-socinfo-nvmem-ddrlp-ipa.patch || die
	eapply "${FILESDIR}"/7.0.x/90020-cix-3aad824-cix-fix-module-modpost-exports.patch || die
	eapply "${FILESDIR}"/7.0.x/90030-cix-cpu-ipa-use-dtb-register-size.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/30110-cix-acpi-ids-and-clkt-consumer-fixes.patch || die
	...
	eapply "${FILESDIR}"/7.0.x/73000-cix-hda-require-cadence-gpio-on-acpi-systems.patch || die
