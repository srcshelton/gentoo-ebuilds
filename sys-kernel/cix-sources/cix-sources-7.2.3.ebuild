...
KEYWORDS="arm arm64"
IUSE="+acpi-table-upgrade +acpi-table-upgrade-dsdt +acpi-table-upgrade-iort-httu +acpi-table-upgrade-iort-msi +radxa-menu"
REQUIRED_USE="
...
# Gentoo has not yet published linux-headers-7.2.  A kernel source package does
# not require matching userspace UAPI headers to build, so retain 7.1 as the
# bounded fallback until the matching headers package is available.
COMMON_DEPEND="
	sys-libs/binutils-libs
	|| (
		~sys-kernel/cix-headers-${KV_MAJOR}.${KV_MINOR}
		~sys-kernel/linux-headers-${KV_MAJOR}.${KV_MINOR}
		~sys-kernel/cix-headers-7.1
...
PATCHES=(
	...
	"${FILESDIR}"/linux-7.1-cifs-fix-softdep-declarations.patch
...
src_prepare() {
	...
	cix_apply_patch() {
		...
		case "${pf##*/}" in
			00101-drm-linlondp-merge-updates-from-26q2.patch)
				# Retain the vendor Linlon/DPTX update without its obsolete
				# generic DRM print-format hunk; 70150 completes the audited
				# 64-plane core conversion against Linux 7.2.
				eapply "${FILESDIR}"/7.2.x/00101-drm-linlondp-merge-updates-from-26q2.patch || die
				return
				;;
			1009-linlondp-fix-WERROR.patch)
				# Keep the CIX display warning fixes without restoring the generic
				# DRM hunk already present in Linux 7.2 or hiding failed PHY setup.
				eapply "${FILESDIR}"/7.2.x/1009-drm-cix-fix-build-warnings.patch || die
				return
				;;
			0001-mailbox-add-acpi-support-to-cix-mailbox-driver.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0001-mailbox-cix-add-audited-acpi-support.patch || die
				return
				;;
			0003-firmware-arm_scmi-add-acpi-support-to-SCMI.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0003-firmware-arm-scmi-add-audited-acpi-support.patch || die
				return
				;;
			0002-acpi-Add-a-property-reference-count-interface.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0002-acpi-cix-resolve-legacy-graph-references.patch || die
				return
				;;
			0004-clk-clk-scmi-register-clkdev-for-acpi.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0004-clk-scmi-add-audited-acpi-publication.patch || die
				return
				;;
			00050-dt-bindings-soc-cix-add-sky1-audss-cru-controller.patch)
				# Consolidate the complete AUDSS/CLKT/CLKA/syscon clock series at
				# its first vendor ordinal so later pieces cannot publish prefixes.
				eapply "${FILESDIR}"/7.2.x/0005-clk-cix-add-audited-sky1-support.patch || die
				return
				;;
			00051-clk-cix-add-sky1-audss-clock-controller.patch|\
			00052-clk-add-cix-clk-driver.patch|\
			00053-clk-cix-add-acpi-support-to-sky1-audss-clock-control.patch)
				# Absorbed by the stable logical 0005 replacement above.
				return
				;;
			00060-reset-cix-add-sky1-audss-auxiliary-reset-driver.patch)
				# Apply the complete audited system/FCH/AUDSS reset model once at
				# the first vendor reset-series ordinal.
				eapply "${FILESDIR}"/7.2.x/0006-reset-cix-add-audited-sky1-support.patch || die
				return
				;;
			00061-reset-sky1-add-acpi-support.patch|\
			00062-reset-cix-add-acpi-support-to-sky1-audss-reset-drive.patch)
				# Absorbed by the stable logical 0006 replacement above.
				return
				;;
			0007-soc-add-cix-acpi-resource-lookup-driver.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0007-soc-cix-harden-acpi-resource-lookup-driver.patch || die
				return
				;;
			0008-pmdomain-add-acpi-support-to-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0008-pmdomain-add-audited-acpi-scmi-support.patch || die
				return
				;;
			...
			0023-syscon-add-device_syscon_regmap_lookup_by_property.patch)
				# The vendor syscon helper is absorbed by 0005. Reuse its now-free
				# ordinal for the independent SOF debugfs lifetime correction.
				eapply "${FILESDIR}"/7.2.x/0023-sound-sof-clean-up-debugfs-lifetime.patch || die
				return
				;;
			0018-clk-clkdev-increase-clkdev-MAX_CON_ID-from-16-to-32.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0018-clk-clkdev-accept-long-identifiers.patch || die
				return
				;;
			0025-usb-add-usb-cdns3-driver-for-cix-soc.patch)
				# Integrate the Sky1 wrapper with the unified Cadence USBSS/USBSSP
				# core, balanced PHY/reset/clock ownership and retryable PM.
				eapply "${FILESDIR}"/7.2.x/0025-usb-cdns3-add-audited-sky1-platform-support.patch || die
				return
				;;
			0026-typec-add-rts5453-driver.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0026-usb-typec-rts5453-add-audited-driver.patch || die
				return
				;;
			0027-soc-add-cix-acpi-usb-scan-handler.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0027-soc-cix-arbitrate-acpi-usb-models.patch || die
				return
				;;
			0029-usb-cdns3-fix-spin-lock-issue-in-suspend-resume.patch)
				# The unified 7.2 replacement 0025 absorbs the role and platform PM
				# error propagation while adapting to the consolidated Cadence core.
				return
				;;
			...
			0068-DPTSW-26453-usb-cdns3-sky1-add-phy-dep-check-at-prob.patch)
				...
				return
				;;
			0071-DPTSW-26459-ALSA-hda-realtek-suppress-auto-mic-on-CI.patch)
				# The audited HDA replacement 0013 owns the complete CIX board
				# topology, including the dual-ADC auto-mic policy.
				return
				;;
			0013-DPTSW-26459-sound-hda-rework-cix-ipbloq-based-on-mai.patch)
				# Consolidate the CIX controller around standard firmware DMA
				# ranges and harden the complete probe/PM ownership model.
				eapply "${FILESDIR}"/7.2.x/0013-sound-hda-cix-add-audited-sky1-support.patch || die
				return
				;;
			0014-kernel-dma-Export-dma_declare_coherent_memory-for-mo.patch)
				# The consolidated HDA driver has no private coherent-pool user.
				return
				;;
			0009-remoteproc-add-cix-dsp-remoteproc-driver.patch)
				# Replace the unsafe vendor transport with the audited,
				# ACPI-capable remoteproc/RPMsg implementation.  It remains
				# dormant until an explicit remoteproc boot request.
				eapply "${FILESDIR}"/7.2.x/0009-remoteproc-cix-sky1-add-audited-hifi5-support.patch || die
				return
				;;
			0020-firmware-add-cix-dsp-ipc-driver.patch)
				# The vendor private IPC helper is unsafe and duplicates the
				# standard SOF mailbox path used by the audited owner below.
				return
				;;
			0021-sound-soc-add-cix-sof-driver.patch)
				# Replace the DT-only vendor owner with the ACPI-native SOF
				# implementation. The replacement includes the fixed 4 KiB IPC3
				# host-page ABI and makes the CIX driver exclusive with XAF.
				eapply "${FILESDIR}"/7.2.x/0021-sound-soc-cix-sky1-add-audited-sof-support.patch || die
				return
				;;
			0016-dma-arm-dma350-add-acpi-support-for-cix-soc.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0016-dma-arm-dma350-add-audited-cix-support.patch || die
				return
				;;
			0017-gpio-add-acpi-support-to-cadence-driver.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0017-gpio-cadence-add-audited-cix-sky1-support.patch || die
				return
				;;
			0019-i2c-add-acpi-support-for-cadence-driver.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0019-i2c-cadence-add-audited-acpi-support.patch || die
				return
				;;
			0044-pwm-sky1-check-pwm-state-before-enable-disable-clk-i.patch)
				# Keep the Sky1 provider, but validate firmware state and balance
				# clocks across probe, apply, suspend, resume and removal.
				eapply "${FILESDIR}"/7.2.x/0044-pwm-sky1-harden-lifecycle-and-state-validation.patch || die
				return
				;;
			0067-PROJ031-2-backlight-pwm-backlight-add-acpi-support-f.patch)
				# Retain firmware-node backlight discovery without bypassing the
				# PWM, regulator, GPIO or deferred-probe ownership contracts.
				eapply "${FILESDIR}"/7.2.x/0067-backlight-pwm-add-safe-firmware-node-support.patch || die
				return
				;;
			00111-drm-panthor-add-ACPI-CIXH5000-platform-binding.patch)
				# Replace the vendor import at its stable logical position.
				eapply "${FILESDIR}"/7.2.x/0011-drm-panthor-add-sky1-acpi-support.patch || die
				return
				;;
			00112-drm-panthor-Sky1-CIXH5000-ACPI-bring-up.patch|\
			00113-panthor-set-DPM_FLAG_NO_DIRECT_COMPLETE-for-STR-on-s.patch|\
			00114-drm-panthor-rework-bus-coherency-to-ACE-Lite-ACE-NON.patch|\
			00115-pmdomain-scmi-expose-scmi_perf_domain_freq_get-helpe.patch|\
			00116-drm-panthor-SCMI-DVFS-via-the-perf-genpd-on-Sky1.patch)
				# Absorbed into the audited 0011, 30200 and 70200 series.
				return
				;;
			0012-irqchip-add-cix-sky1-pdc-driver.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0012-irqchip-cix-sky1-pdc-add-audited-wake-domain.patch || die
				return
				;;
			0024-phy-add-cix-phy-driver.patch)
				# Restore only the audited CIXH2033 USB/DisplayPort provider;
				# PCIe, USB2 and USB3 remain separate deferred workstreams.
				eapply "${FILESDIR}"/7.2.x/0024-phy-cix-add-audited-usbdp-combo-phy.patch || die
				return
				;;
			...
			0048-pinctrl-sky1-add-acpi-support.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0048-pinctrl-sky1-add-audited-acpi-support.patch || die
				return
				;;
			0060-ACPI-thermal-bind-devfreq-cooling-devices-via-devfre.patch)
				...
				eapply "${FILESDIR}"/7.2.x/0060-acpi-thermal-bind-devfreq-cooling-devices-safely.patch || die
				return
				;;
			...
			0070-DPTSW-26437-iommu-arm-smmu-v3-fix-IRQ-setup-on-resum.patch)
				# Retain standard PNP0A08 roots and exclude unsafe vendor-only
	...
	post_vendor_patches=(
		...
		"${FILESDIR}"/7.2.x/20065-cacheinfo-share-global-firmware-ids-across-levels.patch
		"${FILESDIR}"/7.2.x/20070-resctrl-mpam-expose-proportional-bandwidth.patch
		...
		"${FILESDIR}"/7.2.x/30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch
		"${FILESDIR}"/7.2.x/30128-acpi-thermal-retain-downstream-improvements.patch
		"${FILESDIR}"/7.2.x/30129-thermal-cix-add-safe-ipa-support.patch
		...
		"${FILESDIR}"/30200-pmdomain-read-provider-performance-state.patch
		"${FILESDIR}"/7.2.x/40042-platform-acpi-resolve-named-irq-resources.patch
		# CIX-QUESTIONED-PATCH: stock-firmware log demotion; upgraded DSDTs remove the duplicate.
		"${FILESDIR}"/7.2.x/40046-acpi-demote-cix-sky1-ecam-duplicate-reservations.patch
		"${FILESDIR}"/7.2.x/40052-pinctrl-sky1-validate-o6-camera-mclk-duplication.patch
		"${FILESDIR}"/40055-gpio-pinctrl-enforce-sky1-pad-ownership.patch
		"${FILESDIR}"/7.2.x/40056-soc-cix-add-safe-bus-performance-domain-driver.patch
		"${FILESDIR}"/7.2.x/40070-soc-cix-arbitrate-acpi-pcie-models.patch
		"${FILESDIR}"/40093-pci-cix-enable-root-port-io-window-assignment.patch
		"${FILESDIR}"/7.2.x/50030-net-bridge-warn-for-missing-netfilter-on-first-device.patch
		"${FILESDIR}"/7.2.x/50060-watchdog-sbsa-gwdt-use-cix-sky1-refresh-value.patch
		"${FILESDIR}"/7.2.x/50130-spi-cadence-add-audited-cix-acpi-support.patch
		"${FILESDIR}"/7.2.x/60010-usb-cdns3-sky1-declare-phy-dependency.patch
		"${FILESDIR}"/7.2.x/70020-drm-cix-gate-virtual-encoder-build.patch
		...
		"${FILESDIR}"/7.2.x/70130-drm-cix-retain-safe-display-improvements.patch
		"${FILESDIR}"/7.2.x/70135-drm-cix-remove-unsafe-engineering-interfaces.patch
		"${FILESDIR}"/7.2.x/70140-drm-cix-fix-gcc15-clang21-w1-findings.patch
		"${FILESDIR}"/7.2.x/70150-drm-support-up-to-64-planes.patch
		...
		"${FILESDIR}"/7.2.x/70200-drm-panthor-declare-scmi-perf-softdep.patch
		"${FILESDIR}"/7.2.x/70990-media-cix-import-and-integrate-mvx-vpu-driver.patch
		...
		"${FILESDIR}"/7.2.x/71080-cix-mvx-uplift-selected-2026q2-fixes.patch
		"${FILESDIR}"/7.2.x/71090-cix-mvx-uplift-p1-7.0-v1.0.2-core-fixes.patch
		"${FILESDIR}"/7.2.x/71100-cix-mvx-uplift-p1-7.0-v1.0.2-session-api.patch
		"${FILESDIR}"/7.2.x/71110-cix-mvx-fix-source-quality.patch
		"${FILESDIR}"/7.2.x/71120-cix-mvx-harden-dma-firmware-lifetime-and-devfreq.patch
		"${FILESDIR}"/7.2.x/71130-cix-mvx-remove-unsafe-kernel-buffer-dump.patch
		# CIX-QUESTIONED-PATCH: disabled; see the patch preamble.
		# "${FILESDIR}"/7.2.x/71140-cix-mvx-add-rrc-dqp-session-controls.patch
		"${FILESDIR}"/7.2.x/71150-cix-mvx-fix-single-planar-dmabuf-capture.patch
		"${FILESDIR}"/7.2.x/71160-cix-mvx-harden-fault-and-lifecycle-recovery.patch
		"${FILESDIR}"/7.2.x/71500-misc-armchina-npu-import-sky1-driver.patch
		"${FILESDIR}"/7.2.x/71510-misc-armchina-npu-harden-raw-register-io.patch
		"${FILESDIR}"/71520-misc-armchina-npu-restrict-to-cix-sky1-v3.patch
		"${FILESDIR}"/7.2.x/71530-misc-armchina-npu-harden-ownership-and-domains.patch
		"${FILESDIR}"/7.2.x/71540-misc-armchina-npu-add-scmi-opp-devfreq.patch
		"${FILESDIR}"/7.2.x/71550-misc-armchina-npu-balance-runtime-pm.patch
		"${FILESDIR}"/7.2.x/71560-misc-armchina-npu-harden-userspace-abi-and-dma.patch
		"${FILESDIR}"/7.2.x/71570-misc-armchina-npu-harden-fault-teardown-and-source-quality.patch
		"${FILESDIR}"/7.2.x/71580-misc-armchina-npu-link-devfreq-providers.patch
		...
		"${FILESDIR}"/71695-misc-armchina-npu-annotate-locking-guards.patch
		"${FILESDIR}"/71700-misc-armchina-npu-keep-v3-tcbs-in-asid0.patch
		"${FILESDIR}"/71710-misc-armchina-npu-recover-closing-command-pools.patch
		"${FILESDIR}"/7.2.x/72000-media-cix-import-armcb-isp-driver.patch
		"${FILESDIR}"/7.2.x/72010-media-cix-harden-armcb-isp-platform-subdevices.patch
		"${FILESDIR}"/7.2.x/72015-media-cix-harden-armcb-isp-v4l2-streaming.patch
		"${FILESDIR}"/7.2.x/72020-media-cix-harden-armcb-isp-dma-legacy-abi.patch
		"${FILESDIR}"/7.2.x/72022-media-cix-handle-subdevice-probe-deferral.patch
		"${FILESDIR}"/7.2.x/72025-media-cix-coordinate-ddr-low-power-with-isp.patch
		"${FILESDIR}"/7.2.x/72030-media-cix-port-isp-to-linux-7.2.patch
		"${FILESDIR}"/7.2.x/73050-sound-soc-cix-harden-audio-paths.patch
		"${FILESDIR}"/7.2.x/73060-sound-cix-declare-runtime-provider-dependencies.patch
		...
		"${FILESDIR}"/7.2.x/80015-bluetooth-btrtl-return-register-read-error.patch
		"${FILESDIR}"/7.2.x/80020-rtw89-check-acpi-dsm-before-evaluating.patch
		"${FILESDIR}"/7.2.x/80025-cadence-macb-add-sky1-firmware-matches.patch
		"${FILESDIR}"/7.2.x/80030-net-realtek-import-r8126-driver.patch
		...
		"${FILESDIR}"/7.2.x/80070-pci-disable-aspm-for-sky1-smmu-faulting-endpoints.patch
		"${FILESDIR}"/7.2.x/90040-hwmon-cix-add-safe-acpi-fan-control.patch
		"${FILESDIR}"/7.2.x/90045-cix-close-platform-driver-dependencies.patch
	)
	rm -f "${T}/cix-dptx-extra-stream-clock-policy" \
		"${T}/cix-mvx-rrc-dqp-policy" || die
	for pf in "${post_vendor_patches[@]}"; do
		eapply "${pf}" || die
		case ${pf##*/} in
			70030-*) : > "${T}/cix-dptx-extra-stream-clock-policy" || die ;;
			71140-*) : > "${T}/cix-mvx-rrc-dqp-policy" || die ;;
		esac
	done
	if use radxa-menu; then
		eapply "${FILESDIR}"/7.2.x/90050-arm64-cix-add-radxa-orion-board-profiles.patch || die
		eapply "${FILESDIR}"/7.2.x/90060-arm64-cix-model-orion-acpi-infrastructure.patch || die
	fi
	eapply \
		"${FILESDIR}"/7.2.x/90096-soc-cix-add-firmware-scratch-diagnostics.patch \
		"${FILESDIR}"/7.2.x/90098-pstore-ramoops-parse-firmware-node-properties.patch || die
		...
		for board in "${boards[@]}"; do
			for firmware in "${firmware_profiles[@]}"; do
				...
				if use acpi-table-upgrade-dsdt && _cix_acpi_has_dsdt_profile "${board}" "${firmware}"; then
					...
					# MPAM.aml references the replacement PPTT's shared-cache
					# identity, so include it only in that complete profile.
					# It remains inert unless the kernel has ARM64 MPAM enabled.
					if [[ -d "${acpi_src}/shared/${firmware}/mpam" ]]; then
						for scope in "${source_dirs[@]}"; do
							[[ -d "${scope}/mpam" ]] || continue
							for file in "${scope}"/mpam/*.asl; do
								[[ -e "${file:-}" ]] || continue
								_src_compile_asl "${file}" \
									"${board_dst}/initramfs-dsdt/kernel/firmware/acpi"
							done
...
src_install() {
	...
	# e.g. linux-7.2.3 -> linux-7.2.3-cix-r1
	if [[ "${PR:-"r0"}" != 'r0' ]]; then
		kernel_dir="${kernel_dir}-${PR}"
	fi
	...
	for policy_marker in cix-dptx-extra-stream-clock-policy \
		cix-mvx-rrc-dqp-policy; do
...
pkg_postinst() {
	...
	if use acpi-table-upgrade; then
		...
		if use acpi-table-upgrade-dsdt; then
			...
			elog "is qualified."
			elog "The Linux 7.2 O6 and O6N firmware-1.2 DSDT profiles also"
			elog "contain MPAM.aml. It is inert unless CONFIG_ARM64_MPAM and"
			elog "CONFIG_RESCTRL_FS are enabled."
			elog
		fi
