...
- Firmware `1.2` DSDT/whole-table profile: enable the full replacement profile
  ...
  measured-RPM bytes, and auto/manual mode readback. On O6N, the DSDT source
  is the O6N-compatible base DSDT captured from an O6N `1.2.1` system so the
  full profile can share the whole-table payload structure without using the
  O6 DSDT or O6 board SSDT on O6N hardware. Both boards also include the
...
- Firmware `1.3` O6 DSDT profile: enable through
  ...
  same three USB VBUS pin-group consumers as the `1.2` replacement and carrying
  the same fail-closed, read-only EC fan diagnostics. It deliberately does not
...
| O6 | `dsdt only` | `ORIONO6.aml` | firmware-specific `ssdt-replacement/ORIONO6.asl` | USB VBUS regulators, pinctrl, EC fan diagnostics | Replaces the stock O6 board SSDT with an otherwise equivalent OEM-revision-`2` table whose `VUS0`, `VUS4`, and `VUS5` regulators consume `pinctrl_usb0`, `pinctrl_usb4`, and `pinctrl_usb5`, matching the groups published by `MUX1`. Both firmware-specific sources add independent read-only `CIXHA024` methods for target RPM, raw measured-RPM bytes, the supported fan-mode command-version mask, and auto/manual mode readback. Every method fails closed to an unavailable sentinel, so unsupported EC firmware preserves the existing PWM-only interface. The vendor board-table source is unchanged from firmware `1.2.1` through `1.2.4`, while firmware `1.3` uses a separate source derived from its changed stock table. The revision bump is required because Linux accepts an initrd table as an upgrade only when its OEM revision is newer than the firmware table. |
