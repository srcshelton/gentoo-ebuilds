...
| --- | --- | --- | --- |
| [Linux 6.18.48](https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/tag/?h=v6.18.48) | `cix-sources-6.18.48` | `6.18-55` base, extras, and experimental | [`cix-linux-main` at `bc078a3`](https://github.com/cixtech/cix-linux-main/commit/bc078a383042a6c14b05c56e12390e422f893088) |
| [Linux 7.1.12](https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/tag/?h=v7.1.12) | `cix-sources-7.1.12` | `7.1-15` base and extras | same CIX source |
| [Linux 7.2.2](https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/tag/?h=v7.2.2) | `cix-sources-7.2.2` | `7.2-3` base and extras | same CIX source |
...
| Kernel | `6.18.48`, `7.1.12`, `7.2.2` | The maintained Linux version. |
...
The hardware profiles set broad defaults: `server` selects the core headless
platform, storage, network, USB, power and thermal support; `desktop` adds GPU
and display; and `full` also adds VPU and ISP/camera support. Use
`--graphics-profile` to select `none`, display, GPU, desktop, media, or all of
those driver groups independently. Use `--audio-profile` to select none,
analog HDA, display audio, or every audio path supported by the board. Its
default `auto` mode enables supported physical audio paths, omitting display
audio when the resolved graphics profile has no display pipeline. An explicit
display-audio request also enables that pipeline.

The NPU remains an explicit `--with-npu` choice. An internal
...
firmware device. HiFi5 support remains an explicit choice through
`--enable-hifi5-dsp {xaf,sof}`. XAF provides the CIX Audio SDK offload
interface; SOF provides the standard Linux SOF/ALSA interface but is limited on
O6 to a no-codec processing topology. SOF requires the full DSDT table upgrade
and firmware installed with the `sof` USE flag.

The ACPI platform profile also makes the upstream GPIO Aggregator available.
It remains inactive until userspace creates an allow-listed virtual gpiochip;
the kernel package does not expose every internal Sky1 GPIO to unprivileged
users. Header pins which overlap enabled I2C, PWM, UART, SPI, audio, camera, or
touchscreen functions must not be included in such an allow-list. The profile
enables the current GPIO character-device ABI used by libgpiod; it does not
enable the deprecated `GPIO_CDEV_V1` compatibility ABI.
...
preserves an end-user's independent `CONFIG_EXPERT` and
`CONFIG_DEBUG_KERNEL` choices when the profile is later disabled. The retained
Kconfig policy makes those choices independent: `CONFIG_EXPERT` still suggests
the historical debug default, but it no longer overrides an explicit
`CONFIG_DEBUG_KERNEL=n` choice.
...
needs them is omitted. The same Kconfig policy exposes `CONFIG_SLUB_DEBUG`
independently of `CONFIG_EXPERT`, allowing the helper to disable its support
code for a normal production profile. On an unpatched source tree where that
choice remains hidden, the helper preserves Kconfig's default unless expert
controls are already enabled.

## Included changes

The table below names every non-Gentoo patch identity included by the default
maintained configurations. “All” means Linux 6.18.48, 7.1.12, and 7.2.2.
...
| --- | --- | --- | --- |
| 6.18.48 | `1000_linux-6.18.1.patch` through `1047_linux-6.18.48.patch` | `1510_fs-enable-link-security-restrictions-by-default.patch`<br>`1700_sparc-address-warray-bound-warnings.patch`<br>`1730_parisc-Disable-prctl.patch`<br>`2000_BT-Check-key-sizes-only-if-Secure-Simple-Pairing-enabled.patch`<br>`2901_permit-menuconfig-sorting.patch`<br>`2920_sign-file-patch-for-libressl.patch`<br>`2990_libbpf-v2-workaround-Wmaybe-uninitialized-false-pos.patch`<br>`2991_libbpf_add_WERROR_option.patch`<br>`3000_Support-printing-firmware-info.patch`<br>`4567_distro-Gentoo-Kconfig.patch` | Updates Linux 6.18.0 to 6.18.48 and applies the pinned Gentoo defaults and compatibility fixes. The updated Gentoo Kconfig patch makes the former local ARM64 dependency workaround unnecessary. |
| 7.1.12 | `1001_linux-7.1.1.patch`, `1001_linux-7.1.2.patch`, and `1002_linux-7.1.3.patch` through `1011_linux-7.1.12.patch` | `1510_fs-enable-link-security-restrictions-by-default.patch`<br>`1700_sparc-address-warray-bound-warnings.patch`<br>`1710_x86-tools-vdso2c.patch`<br>`1730_parisc-Disable-prctl.patch`<br>`2000_BT-Check-key-sizes-only-if-Secure-Simple-Pairing-enabled.patch`<br>`2901_permit-menuconfig-sorting.patch`<br>`2902_Replace-CONST-CAST-with-const-cast.patch`<br>`2990_libbpf-v2-workaround-Wmaybe-uninitialized-false-pos.patch`<br>`2991_libbpf_add_WERROR_option.patch`<br>`3000_Support-printing-firmware-info.patch`<br>`4567_distro-Gentoo-Kconfig.patch` | Updates Linux 7.1.0 to 7.1.12 and applies the pinned Gentoo security, build, and configuration fixes. The SCMI zero-vote and Panthor zero-sized-firmware-section fixes from 7.1.10 remain intact alongside the CIX adaptations. |
| 7.2.2 | `1000_linux-7.2.1.patch` and `1001_linux-7.2.2.patch` | `1510_fs-enable-link-security-restrictions-by-default.patch`<br>`1700_sparc-address-warray-bound-warnings.patch`<br>`1710_x86-tools-vdso2c.patch`<br>`1730_parisc-Disable-prctl.patch`<br>`2000_BT-Check-key-sizes-only-if-Secure-Simple-Pairing-enabled.patch`<br>`2901_permit-menuconfig-sorting.patch`<br>`2902_Replace-CONST-CAST-with-const-cast.patch`<br>`2990_libbpf-v2-workaround-Wmaybe-uninitialized-false-pos.patch`<br>`2991_libbpf_add_WERROR_option.patch`<br>`3000_Support-printing-firmware-info.patch`<br>`4567_distro-Gentoo-Kconfig.patch` | Updates Linux 7.2.0 to 7.2.2 and applies the pinned Gentoo security, build, and configuration fixes. |
...
| Clocks and resets | `0004-clk-scmi-add-audited-acpi-publication.patch`<br>`0005-clk-cix-add-audited-sky1-support.patch`<br>`0006-reset-cix-add-audited-sky1-support.patch`<br>`0018-clk-clkdev-increase-clkdev-MAX_CON_ID-from-16-to-32.patch`<br>`0018-clk-clkdev-accept-long-identifiers.patch`<br>`linux-6.18-clk-composite-export-devm-pdata-helper.patch` | Core files and composite-helper backport: all; the two `0018` forms apply to 6.18/7.1 and 7.2 respectively | Publishes SCMI clocks to ACPI consumers and supplies the Sky1 clock and reset controllers, including longer firmware clock names. The accepted upstream composite-clock export lets the audio clock controller remain modular. Retryable clock-provider deferral is handled without a false invalid-CLKT error, overlapping reset/syscon resources are matched by exact CIX provider identity rather than address alone, and intentionally overlapping child register windows are mapped without claiming exclusive ownership. |
...
| Build compatibility | `10020-lld-timer-of-table-end-warning.patch`<br>`10050-bpf-gate-struct-ops-on-kallsyms.patch`<br>`10070-kconfig-separate-expert-and-debug-policy.patch` | All | Removes an ACPI-only linker warning, fixes configuration-dependent BPF build failures, and exposes BPF struct-ops providers only when their BTF and kallsyms requirements are present. `10070` lets expert configuration controls, generic kernel debugging, and SLUB debugging be selected independently. Ordinary BPF and non-struct-ops networking remain available without kallsyms. |
...
| CPU frequency and topology | `20050-topology-has-missing-cpufreq-ref.patch`<br>`20060-acpi-processor-clarify-ignore-ppc-module-parameter.patch`<br>`20065-cacheinfo-share-global-firmware-ids-across-levels.patch` | All | Gives the scheduler a valid ACPI/CPPC frequency reference and clarifies the `_PPC` override control. `20065` lets Linux recognise one physical cache from its globally unique ACPI PPTT Cache ID even when heterogeneous CPUs reach it at different relative levels; on Sky1 this makes the 12 MiB Hayden/DSU cache's system-wide sharing visible instead of splitting the A720 and A520 CPU maps. Linux 7.1 and 7.2 also assign that cache a deterministic highest relative level so the MPAM table can reference it without depending on CPU enumeration order. The sharing correction changes cache reporting rather than cache allocation or scheduling policy; allocation changes only when the end user enables arm64 MPAM. |
| MPAM resource control | `20070-resctrl-mpam-expose-proportional-bandwidth.patch` | 7.1 and 7.2 | Adds an explicit `mbw_prop` mount option to Linux's [arm64 MPAM](https://docs.kernel.org/arch/arm64/mpam.html) [resctrl interface](https://docs.kernel.org/filesystems/resctrl.html) for the DSU-120's work-conserving proportional-bandwidth control. The `MB_PROP` schema exposes the hardware's raw stride-minus-one values from 0 to 63; an ordinary resctrl mount remains unchanged. Non-zero values lower a group's relative bandwidth share under contention and produce a one-time warning because their performance effect is workload-dependent. |
| Thermal and cooling | `0042-thermal-set-thermal_zone-type-from-firmware-in-acpi_.patch`<br>`0060-acpi-thermal-bind-devfreq-cooling-devices-safely.patch`<br>`30090-scmi-hwmon-do-not-use-of-thermal-zones-on-acpi.patch`<br>`30127-acpi-thermal-filter-orion-o6-ectz-zero-readings.patch`<br>`30128-acpi-thermal-retain-downstream-improvements.patch`<br>`30129-thermal-cix-add-safe-ipa-support.patch` | All; `30127` also covers `TZN0` and `TZN1` on 7.2 | Names thermal zones, retains SCMI sensors, rejects bogus EC readings, binds devfreq cooling safely, and supplies corrected CIX power allocation with a bounded standard-ACPI fallback. Linux 7.2 also retries the all-ones `_TMP` failure returned transiently by `TZN0` and `TZN1`, retains a recent valid sample during a short outage, and reports a persistent invalid value as unavailable. Missing optional CIX power metadata is reported at informational level; malformed metadata remains a warning. |
| EFI RTC alarm | `linux-6.18-rtc-efi-restore-verified-wakeup-alarm.patch` | All | Restores the standard RTC wake-alarm interface only when firmware both advertises and successfully answers its EFI wakeup-time service. O6/O6N alarms are exposed with the HYM8563's minute resolution while the I2C device remains under firmware ownership. This prepares userspace alarm handling; successful system wake still requires working platform suspend/resume firmware. |
| Wake interrupts, DMA, and GPIO | `0012-irqchip-cix-sky1-pdc-add-audited-wake-domain.patch`<br>`0016-dma-arm-dma350-add-audited-cix-support.patch`<br>`0017-gpio-cadence-add-audited-cix-sky1-support.patch`<br>`40055-gpio-pinctrl-enforce-sky1-pad-ownership.patch` | All | Adds the Sky1 wake controller, DMA350 operation, and ACPI-aware Cadence GPIO needed by board peripherals and suspend/resume. The sparse ACPI GPIO-to-pad map makes GPIO requests participate in pinctrl ownership, restores firmware bypass state on release, and prevents a GPIO consumer from silently stealing a pad claimed by a peripheral. DMA1 accepts the native-firmware compatibility map or the standard table-upgrade `_DMA` range and fails safely rather than transferring through an unspecified address window. |
| Pinctrl | `00481-pinctrl-cix-Add-pin-controller-support-for-sky1.patch`<br>`00482-pinctrl-cix-sky1-Provide-pin-control-dummy-states.patch`<br>`00483-pinctrl-cix-Fix-obscure-dependency.patch`<br>`00484-pinctrl-cix-sky1-Unexport-sky1_pinctrl_pm_ops.patch`<br>`0048-pinctrl-sky1-add-audited-acpi-support.patch`<br>`40052-pinctrl-sky1-validate-o6-camera-mclk-duplication.patch`<br>`40055-gpio-pinctrl-enforce-sky1-pad-ownership.patch` | `00481`-`00484`: 6.18 only; remainder: all | Provides Sky1 pin multiplexing and ACPI integration, including compatibility with the USB VBUS group names in released stock Radxa tables and strict exclusion between peripheral and GPIO ownership. Corrected table-upgrade profiles publish dedicated VBUS GPIO groups instead. A narrow compatibility patch removes pin 65 from the second camera group only when both groups contain the exact known O6/O6N MCLK tuple; unexpected firmware still fails visibly. The four 6.18 files backport prerequisites already present in 7.x. |
| I2C and SPI | `0019-i2c-cadence-add-audited-acpi-support.patch`<br>`50130-spi-cadence-add-audited-cix-acpi-support.patch` | All | Enables the Cadence I2C and classic SPI controllers from ACPI with validated firmware limits and correct clock/reset deferral. Linux 6.18 also exposes its new Sky1 16- and 32-bit classic-SPI transfers to the ACPI-described controller rather than only to the DT compatible. |
| USB and Type-C | `0024-phy-cix-add-audited-usbdp-combo-phy.patch`<br>`0025-usb-cdns3-add-audited-sky1-platform-support.patch`<br>`0026-usb-typec-rts5453-add-audited-driver.patch`<br>`0027-soc-cix-arbitrate-acpi-usb-models.patch`<br>`0029-usb-cdns3-propagate-role-pm-errors.patch`<br>`60010-usb-cdns3-sky1-declare-phy-dependency.patch` | Core files: all; `0029`: 6.18 and 7.1, with its change included in `0025` on 7.2 | Adds the CIXH2033 USB/DisplayPort combo PHY, Sky1 Cadence USB wrapper, and RTS5453 Type-C controller; gives each of the two O6 port functions of one RTS5453H its own handler on their shared interrupt line; selects safely between duplicate firmware models; and reports role/PM failures. The wrapper selects its required USB/DP PHY and asks the module loader to load that provider first. RTS5453 cable orientation is passed through the Type-C switch to the CIXH2033 PHY; the fixed-host exception only avoids requiring an unused OTG role switch. The unrelated vendor PCIe, USB2, and USB3 PHY implementations remain excluded. Reorientation while the PHY is active is rejected; disconnect and reconnect the cable to change orientation. |
...
| Linlon display and DisplayPort | `00100-drm-add-cix-linlon-dp-driver.patch`<br>`00101-drm-linlondp-merge-updates-from-26q2.patch`<br>`0043-DPTSW-19618-linlon-dp-Set-AFBC-32x8-to-the-highest-p.patch`<br>`1001-linlondp-fix-build-of-debugfs.patch`<br>`1002-linlondp-add-missing-headers.patch`<br>`1003-linlondp-add-api-fix-up-to-6.18.patch`<br>`1004-linlondp-disable-enable_render-by-default.patch`<br>`1005-linlondp-set-DRM_FBDEV_DMA_DRIVER_OPS-for-linlondp-kms-driver.patch`<br>`1006-drm-panel-add-fwnode_drm_find_panel.patch`<br>`1007-fix-26q2-linlondp-for-mainline.patch`<br>`1008-linlondp-add-api-fix-up-to-v7.1.patch`<br>`1009-drm-cix-fix-build-warnings.patch`<br>`70020-drm-cix-gate-virtual-encoder-build.patch`<br>`70030-drm-cix-dptx-make-extra-stream-clocks-optional.patch`<br>`70080-drm-cix-remove-unused-dptx-cadence-phy-kconfig.patch`<br>`70105-drm-cix-linlon-dp-tighten-private-include-flags.patch`<br>`70120-drm-cix-demote-internal-tbu-noop-logs.patch`<br>`70130-drm-cix-retain-safe-display-improvements.patch`<br>`70135-drm-cix-remove-unsafe-engineering-interfaces.patch`<br>`70140-drm-cix-fix-gcc15-clang21-w1-findings.patch`<br>`70150-drm-support-up-to-64-planes.patch`<br>`70160-drm-cix-dptx-fix-audio-eld-and-shutdown.patch`<br>`70195-drm-cix-dptx-fix-mst-lock-leaks.patch` | Core files and `70140`: all; `1008`: 7.1 and 7.2; the 7.2 form of `1009`: 7.2 only | Adds the CIX Linlon display and DP stack, AFBC, panel and fbdev support; adapts it to current kernels and firmware; removes unsafe engineering interfaces; and completes the CIX 64-plane mask conversion. DPTX supplies the sink's ELD to ALSA, programs the selected DisplayPort Audio InfoFrame, and restores it after link reconfiguration; shutdown always disables the audio block even after unplug. MST error paths release the topology lock before returning. Nested ACPI port/endpoint references are resolved by shared patch `0002`, and Linlon fails probe safely if firmware describes no pipeline component. When both the Trilin display and PWM backlight are modular, the display module asks the module loader to load `pwm_bl` first; device links and probe deferral remain the correctness mechanism. |
...
| HDA and ASoC audio | `0013-sound-hda-cix-add-audited-sky1-support.patch`<br>`0022-sound-soc-add-cix-soc-support.patch`<br>`0071-DPTSW-26459-ALSA-hda-realtek-suppress-auto-mic-on-CI.patch`<br>`73050-sound-soc-cix-harden-audio-paths.patch`<br>`73060-sound-cix-declare-runtime-provider-dependencies.patch` | Core files: all; standalone `0071`: 6.18 only, with its change included in `0013` on 7.1 and 7.2 | Provides the Sky1 HDA controller and CIX ASoC/HDMI-DP paths, applies the board microphone quirk, and hardens DMA, PM, probe, and teardown. Kconfig closes over the required Sky1 clock, reset and DMA providers, while modular consumers request those providers before loading. The generic sound card continues to rely on firmware-described device links for codec and display suppliers so non-display topologies are not forced to load the display stack. |
...
The display stack also applies `70170`, `70180`, and `70190`, which preserve
clock and reset-provider errors and correct AUX retry accounting without
changing the supported display interfaces.
...
- On Linux 7.1 and 7.2, the O6/O6N firmware-1.2 full DSDT profile contains an MPAM
