...
| Kernel | `6.18.44`, `7.1.8`, `7.2.0` | The maintained Linux version. |
...
`CONFIG_DEBUG_KERNEL` choices when the profile is later disabled. Upstream
`CONFIG_EXPERT=y` selects `CONFIG_DEBUG_KERNEL=y`, so the helper does not emit
an impossible request to disable the latter while preserving the former.
...
maintained configurations. “All” means Linux 6.18.44, 7.1.8, and 7.2.0.
...
| USB and Type-C | `0024-phy-cix-add-audited-usbdp-combo-phy.patch`<br>`0025-usb-cdns3-add-audited-sky1-platform-support.patch`<br>`0026-usb-typec-rts5453-add-audited-driver.patch`<br>`0027-soc-cix-arbitrate-acpi-usb-models.patch`<br>`0029-usb-cdns3-propagate-role-pm-errors.patch` | Core files: all; `0029`: 6.18 and 7.1, with its change included in `0025` on 7.2 | Adds the CIXH2033 USB/DisplayPort combo PHY, Sky1 Cadence USB wrapper, and RTS5453 Type-C controller; gives each of the two O6 port functions of one RTS5453H its own handler on their shared interrupt line; selects safely between duplicate firmware models; and reports role/PM failures. RTS5453 cable orientation is passed through the Type-C switch to the CIXH2033 PHY; the fixed-host exception only avoids requiring an unused OTG role switch. The unrelated vendor PCIe, USB2, and USB3 PHY implementations remain excluded. Reorientation while the PHY is active is rejected; disconnect and reconnect the cable to change orientation. |
...
| HDA and ASoC audio | `0013-sound-hda-cix-add-audited-sky1-support.patch`<br>`0022-sound-soc-add-cix-soc-support.patch`<br>`0071-DPTSW-26459-ALSA-hda-realtek-suppress-auto-mic-on-CI.patch`<br>`73050-sound-soc-cix-harden-audio-paths.patch` | Core files: all; standalone `0071`: 6.18 only, with its change included in `0013` on 7.1 and 7.2 | Provides the Sky1 HDA controller and CIX ASoC/HDMI-DP paths, applies the board microphone quirk, and hardens DMA, PM, probe, and teardown. |
