...
src_prepare() {
	...
	for pf in "${cix_patch_dir}"/*.patch; do
		case "$( basename "${pf}" )" in
			'0005-clk-add-cix-clk-driver.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0005-cix-clk-add-cix-clk-driver.patch || die
				;;
			'0006-reset-add-cix-reset-driver.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0006-cix-reset-add-cix-reset-driver.patch || die
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0007-reset-core-drop-unused-optional-local.patch || die
				;;
			'0008-pmdomain-add-acpi-support-to-cix-soc.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0008-cix-pmdomain-add-acpi-support-to-cix-soc.patch || die
				;;
			'0011-drm-panthor-add-acpi-support-for-cix-p1.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0011-cix-drm-panthor-add-acpi-support-for-cix-p1.patch || die
				;;
			'0024-phy-add-cix-phy-driver.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0024-cix-phy-add-cix-phy-driver.patch || die
				;;
			'0033-regulator-add-acpi-support.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0033-cix-regulator-add-acpi-support.patch || die
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0034-regulator-fwnode-fix-coupled-count-error-check.patch || die
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0035-regulator-core-brace-acpi-coupling-check.patch || die
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0036-regulator-fwnode-drop-unused-supply-helper.patch || die
				;;
			...
			'0048-pinctrl-sky1-add-acpi-support.patch')
				eapply "${FILESDIR}"/cix-759efc0/7.1.x/0048-cix-pinctrl-sky1-add-acpi-support.patch || die
				;;
			*)
				eapply "${pf}" || die
				;;
	...
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/0047-tty-amba-pl011-drop-stale-console-line-ended-write.patch || die
	...
	eapply "${FILESDIR}"/71989-armchina-npu-add-sky1-priv-init-prototype.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/50000-iommu-arm-smmu-v3-add-acpi-boot-active-bypass-stes-for-cix-sky1-pcie.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/20010-cix-fix-deps-section-mismatch-and-clang-uninit-build-fail.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/70010-cix-759efc0-drm-cix-dptx-fix-clang-werror-in-component-bypass-builds.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70011-drm-cix-dptx-drop-unused-match-data-local.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/30000-cix-759efc0-pmdomain-fix-acpi-scmi-perf-domain-wiring.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/30015-cix-759efc0-pmdomain-export-genpd-dev-pm-attach-by-name.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70020-cix-759efc0-cix-display-and-backlight-build-fixes.patch || die
	eapply "${FILESDIR}"/70030-drm-cix-dptx-make-extra-stream-clocks-optional.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/70040-drm-panthor-drop-unused-gem-device-variable.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/70050-cix-759efc0-drm-cix-enable-acpi-stub-fdt-display.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/70070-cix-759efc0-drm-cix-use-fwnode-display-links.patch || die
	eapply "${FILESDIR}"/70080-drm-cix-remove-unused-dptx-cadence-phy-kconfig.patch || die
	eapply "${FILESDIR}"/cix-759efc0/shared/70090-drm-cix-remove-unused-display-kconfig-prompts.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/70100-cix-759efc0-drm-cix-linlon-dp-fix-clang-warnings.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70102-drm-cix-linlon-dp-use-output-color-format-bitmasks.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70103-drm-cix-linlon-dp-port-private-objects-to-state-create.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70104-drm-cix-linlon-dp-fix-werror-warnings.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70106-drm-cix-dptx-use-output-color-format-bitmasks.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70107-drm-cix-dptx-mark-encoder-atomic-check-static.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/70108-drm-cix-dptx-fix-werror-warnings.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/70110-cix-759efc0-drm-cix-demote-display-info-logs.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/shared/40049-pinctrl-sky1-drop-unused-debug-show-data.patch || die
	eapply "${FILESDIR}"/40093-pci-cix-enable-root-port-io-window-assignment.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/40050-cix-759efc0-soc-cix-arbitrate-acpi-usb-models.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/40060-cix-759efc0-soc-cix-add-gpu-cca-scan-quirk.patch || die
	eapply "${FILESDIR}"/7.1.x/40070-soc-cix-arbitrate-acpi-pcie-models.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/80092-pci-cadence-declare-sky1-config-read-helpers.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/90000-cix-759efc0-soc-cix-add-acpi-bus-perf-driver.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/90010-cix-759efc0-cix-sky1-acpi-socinfo-nvmem-ddrlp-ipa.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/90020-cix-759efc0-cix-fix-module-modpost-exports.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/90098-pstore-ramoops-parse-firmware-node-properties.patch || die
	eapply "${FILESDIR}"/7.0.x/60010-usb-cdnsp-sky1-fix-acpi-fwnode-and-pm-paths.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/60015-cix-759efc0-usb-cdnsp-sky1-tear-down-host-on-shutdown.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/30110-7.0.9-cix-acpi-ids-and-clkt-consumer-fixes.patch || die
	...
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/30105-clk-sky1-audss-use-determine-rate.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/73000-cix-hda-require-cadence-gpio-on-acpi-systems.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.0.x/73010-cix-hda-prefer-acpi-dma-ranges-and-harden-probe.patch || die
	eapply "${FILESDIR}"/cix-759efc0/7.1.x/50010-gpio-cadence-restore-match-data-and-skip-init.patch || die
...
pkg_postinst() {
	...
	elog
	elog "These controls take effect only when ACPI/IORT exposes the"
	elog "Sky1 PCIe SMMU at 0x0b010000."
	elog "Stock Radxa firmware 1.3.0 omits that SMMU from IORT, so the"
	elog "kernel accepts these parameters but has nothing to apply them to."
	elog "The parameters cannot enable or reconstruct a missing IORT node."
