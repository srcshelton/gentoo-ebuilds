...
SLOT="${PV#*"_p"}"
IUSE="+64bit +devicetree extras +os-prefix +rpi-all rpi0 rpi02 rpi-cm0 rpi1 rpi-cm rpi2 rpi-cm2 rpi3 rpi-cm3 rpi4 rpi400 rpi-cm4 rpi-cm4s rpi-cm4-io rpi5 rpi500 rpi-cm5 rpi-cm5-io"
REQUIRED_USE="
...
src_install() {
	...
	for f in boot/*.img; do
		case "${f}" in
			...
			boot/kernel7.img)
				einfo "Found '${f}' ..."
				if use rpi-all || use rpi2 || use rpi-cm2 || use rpi3 || use rpi-cm3 || use rpi02 || use rpi-cm0; then
	...
	if [[ -n "${ver}" ]]; then
		...
		if use rpi-all || use rpi4 || use rpi400 || use rpi-cm4 || use rpi-cm4s; then
			if use rpi-all || use 64bit; then
				if [[ -e extra/System8.map ]]; then
					...
					newins extra/System8_rt.map "System.map-${ver}-v8-rt+"
					f="${f:+${f}, }System.map-${ver}-v8+"
					f="${f:+${f}, }System.map-${ver}-v8-rt+"
					files+=( 'extra/System8.map' )
					files+=( 'extra/System8_rt.map' )
				fi
				if use extras; then
					newins extra/Module8.symvers "Module.symvers-${ver}-v8+"
					newins extra/Module8_rt.symvers "Module.symvers-${ver}-v8-rt+"
					newins extra/uname_string8 "uname-${ver}-v8+"
					newins extra/uname_string8_rt "uname-${ver}-v8-rt+"
				fi
			fi
		fi
		if use rpi-all || use rpi4 || use rpi400 || use rpi-cm4 || use rpi-cm4s; then
		...
		if use rpi-all || use rpi1 || use rpi-cm || use rpi0; then
			...
			if use extras; then
				newins extra/Module.symvers "Module.symvers-${ver}+"
				newins extra/uname_string "uname-${ver}+"
			fi
		fi
	...
	# Install Device Tree Blobs and overlays ...
	#
