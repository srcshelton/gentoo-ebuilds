...
REQUIRED_USE="
	...
	rpi-cm5-io? ( rpi-cm5 )
	rt? ( || ( rpi3 rpi-cm3 rpi4 rpi400 rpi-cm4 rpi-cm4s ) )
"
...
IMAGE_2712="${IMAGE_BASE}/linux-image-${PV}+rpt-rpi-2712_${PV}-1+rpt1~bookworm_arm64.deb"
IMAGE_V8="${IMAGE_BASE}/linux-image-${PV}+rpt-rpi-v8_${PV}-1+rpt1~bookworm_arm64.deb"
IMAGE_V8_RT="${IMAGE_BASE}/linux-image-${PV}+rpt-rpi-v8-rt_${PV}-1+rpt1~bookworm_arm64.deb"
IMAGE_V7="${IMAGE_BASE}/linux-image-${PV}+rpt-rpi-v7_${PV}-1+rpt1~bookworm_armhf.deb"
IMAGE_V7L="${IMAGE_BASE}/linux-image-${PV}+rpt-rpi-v7l_${PV}-1+rpt1~bookworm_armhf.deb"
IMAGE_V6="${IMAGE_BASE}/linux-image-${PV}+rpt-rpi-v6_${PV}-1+rpt1~bookworm_armhf.deb"
	...
	for kernel_ext in '2712' 'v8-rt' 'v8' 'v7l' 'v7' 'v6'; do
		case "${kernel_ext}" in
			'2712')
				...
				;;
			'v8-rt')
				if use rpi-all || { use 64bit && use rt && {
...
src_install() {
	...
	if use rpi-all || use rpi4 || use rpi400 || use rpi-cm4 || use rpi-cm4s || use rpi02 || use rpi3 || use rpi-cm3; then
		if use rpi-all || { use 64bit && use rt; }; then
