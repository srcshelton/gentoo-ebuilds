...
LICENSE="Apache-2.0"
SLOT="0"
IUSE="aufs btrfs +contrib +device-mapper doc lxc systemd udev vim-syntax zsh-completion"
...
RDEPEND="
	${CDEPEND}
	!app-containers/docker-bin
	|| (
		>=net-firewall/iptables-1.4
		net-firewall/iptables-nftables
	)
	lxc? (
		>=app-containers/lxc-0.8
	)
	>=dev-vcs/git-1.7
...
pkg_setup() {
	...
	# many of these were borrowed from the app-containers/lxc ebuild
	CONFIG_CHECK+="
		~CGROUPS
		~CGROUP_CPUACCT
		~CGROUP_DEVICE
	...
	for c in GRKERNSEC_CHROOT_MOUNT GRKERNSEC_CHROOT_DOUBLE GRKERNSEC_CHROOT_PIVOT GRKERNSEC_CHROOT_CHMOD; do
		declare "ERROR_$c"="CONFIG_$c: see app-containers/lxc postinst notes for why some GRSEC features make containers unusuable"
	done
...
src_install() {
	...
	use systemd && systemd_dounit contrib/init/systemd/docker.service
