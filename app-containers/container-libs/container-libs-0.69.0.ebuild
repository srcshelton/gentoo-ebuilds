...
src_prepare() {
	...
	if use !tool; then
		eapply "${FILESDIR}"/c-libs-0.68.0-remove-go-cc.patch
	else
		for file in \
...
src_compile() {
	...
	if use tool; then
		...
		emake -C storage GOMD2MAN=go-md2man containers-storage docs
	else
...
src_install() {
	...
	keepdir /etc/containers/{certs.d,oci/hooks.d,networks,registries.conf.d,registries.d}
