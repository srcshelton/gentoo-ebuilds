...
Options:
  ...
  --stop-before-patch P   Materialise the source tree immediately before patch P.
  --patch-log FILE        Write applied patch order, origin, SHA-256, and any
                          intercepted CIX patch as TSV.
...
apply_patch_file() {
	local origin=${2:-external}

	printf 'Applying %s\n' "$1"
	patch --batch --forward --no-backup-if-mismatch -p1 -i "$1" || return
	record_patch_file "$1" "$origin"
}
...
apply_local_patch_file() {
	...
	printf 'Applying %s\n' "$1"
	patch --batch --forward --fuzz=0 --no-backup-if-mismatch -p1 -i "$1" || return
	record_patch_file "$1" overlay "$replaces"
}
...
apply_patch_archive() {
	...
	mkdir -p -- "${T}/patches"
	tar -xf "$archive" -C "${T}/patches"
	while IFS= read -r patch_file; do
		apply_patch_file "$patch_file" "$origin" || return
...
record_patch_file() {
	...
	local origin=$2
	local replaces=${3:-}
	local source=$patch_file

	[[ -n $patch_log_tmp ]] || return 0

	case $patch_file in
		"${FILESDIR}"/*) source=${patch_file#"${repo_root}/"} ;;
		"${WORKDIR}"/cix-linux-main-*) source="cix-linux-main:${patch_file##*/}" ;;
		"${T}"/patches/*) source=${patch_file#"${T}/patches/"} ;;
		*) source=${patch_file##*/} ;;
	esac

	patch_sequence=$((patch_sequence + 1))
	printf '%d\t%s\t%s\t%s\t%s\t%s\n' \
		"$patch_sequence" "$origin" "$(sha256sum "$patch_file" | awk '{ print $1 }')" \
		"${patch_file##*/}" "$replaces" "$source" >> "$patch_log_tmp"
}
...
eapply() {
	...
	for patch_file in "$@"; do
		if [[ -n $stop_before_patch && ${patch_file##*/} == "$stop_before_patch" ]]; then
			printf '%s\n' "$patch_file" > "${T}/stopped-before-patch"
			exit 97
		fi
		case $patch_file in
			"${FILESDIR}"/*)
				replaces=''
				if [[ ${FUNCNAME[1]:-} == cix_apply_patch && -n ${pf:-} ]]; then
					replaces=${pf##*/}
				fi
				apply_local_patch_file "$patch_file" "$replaces" || return
				;;
			"${WORKDIR}"/cix-linux-main-*)
				origin="cix:${EGIT_CIX_COMMIT:-unknown}"
				apply_patch_file "$patch_file" "$origin" || return
				;;
			*) apply_patch_file "$patch_file" external || return ;;
		esac
	done
}
...
while (($#)); do
	case $1 in
		...
		--stop-before-patch)
			shift
			(($#)) || fail "--stop-before-patch requires a patch basename"
			[[ $1 != */* ]] || fail "--stop-before-patch requires a basename, not a path"
			stop_before_patch=$1
			;;
		--patch-log)
			shift
			(($#)) || fail "--patch-log requires a file"
			patch_log=$1
			;;
		-h|--help)
			usage
...
for cmd in awk curl patch sha256sum sha512sum tar; do
...
if [[ -n $patch_log ]]; then
	...
	patch_log=$(cd -- "$(dirname -- "$patch_log")" && pwd -P)/$(basename -- "$patch_log")
	patch_log_tmp="${work_root}/patch-log.tsv"
	printf 'sequence\torigin\tsha256\tbasename\treplaces\tsource\n' > "$patch_log_tmp"
fi
...
for kind in ${K_WANT_GENPATCHES:-}; do
	...
	apply_patch_archive \
		"${DISTDIR}/genpatches-${OKV}-${K_GENPATCHES_VER}.${kind}.tar.xz" \
		"gentoo-genpatches:${OKV}-${K_GENPATCHES_VER}.${kind}"
done

for patch_file in "${UNIPATCH_LIST[@]}"; do
	case $patch_file in
		*.tar|*.tar.*|*.tgz|*.tbz)
			apply_patch_archive "$patch_file" ebuild-unipatch
			;;
		*)
			apply_patch_file "$patch_file" ebuild-unipatch
			;;
	esac
done
...
if [[ -n $stop_before_patch ]]; then
	if [[ -s ${T}/stopped-before-patch && $prepare_status -eq 97 ]]; then
		printf 'Stopped before patch: %s\n' "$(<"${T}/stopped-before-patch")"
	elif [[ $prepare_status -eq 0 ]]; then
		fail "requested stop patch was not encountered: ${stop_before_patch}"
	else
		fail "source preparation failed before requested patch ${stop_before_patch}"
	fi
elif [[ $prepare_status -ne 0 ]]; then
	fail "source preparation failed"
fi

if [[ $compile_acpi_tables == yes && -z $stop_before_patch ]]; then
	if ! ( src_compile ); then
...
if [[ -n $patch_log ]]; then
	mv -- "$patch_log_tmp" "$patch_log"
	printf 'Applied patch log: %s\n' "$patch_log"
fi
