...
IUSE="+compat elogind +kill modern-top +ncurses nls selinux skill static-libs systemd test unicode"
RESTRICT="!test? ( test )"
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		# We keep pidof, ps and kill in /bin per bug #565304.
		dodir /bin
		dodir /sbin
		mv "${ED}"/usr/bin/pidof "${ED}"/bin/ || die
		mv "${ED}"/usr/bin/ps "${ED}"/bin/ || die
		mv "${ED}"/usr/sbin/sysctl "${ED}"/sbin/ || die
		use compat && dosym ../../sbin/sysctl /usr/sbin/sysctl
		if use kill; then
