...
# $Id: a9e1ceb61bb39f7d73d3725d7d64c905a3b3f89d $
...
IUSE="readline static static-libs clvm cman +lvm1 lvm2create_initrd selinux systemd +udev +thin"
...
src_install() {
	local list="install"
	use systemd && list="${list} install_systemd_units install_systemd_generators install_tmpfiles_configuration"
	local inst
	for inst in ${list}; do
	...
	newinitd "${FILESDIR}"/device-mapper.rc-2.02.97-r1 device-mapper
	...
	newins "${FILESDIR}"/dm-start.sh-2.02.97-r1 dm-start.sh || die
