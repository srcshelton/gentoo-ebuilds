...
inherit linux-info tmpfiles usr-ldscript verify-sig
...
IUSE="+argon2 ${CRYPTO_BACKENDS} fips gcrypt kernel nettle nls +openssl passwdqc pwquality ssh static static-libs test +tmpfiles +udev urandom"
RESTRICT="!test? ( test )"
...
pkg_postinst() {
	use tmpfiles && tmpfiles_process cryptsetup.conf
