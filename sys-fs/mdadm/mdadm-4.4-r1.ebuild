...
src_install() {
	...
	if ! use udev; then
		if [[ -d "${ED}$(get_udevdir)"/rules.d ]]; then
			rm -v "${ED}$(get_udevdir)"/rules.d/*.rules
			rmdir -p "${ED}$(get_udevdir)"/rules.d
		elif [[ -d "${ED%/}"/lib/udev/rules.d ]]; then
			rm -v "${ED%/}"/lib/udev/rules.d/*.rules
			rmdir -p "${ED%/}"/lib/udev/rules.d
		fi
		[[ -d "${ED%/}"/lib/udev ]] && rmdir "${ED%/}"/lib/udev
	fi

	# mdadm is boot-critical...
	dodir /sbin
	mv "${ED}"/usr/sbin/mdadm "${ED}"/sbin/ || die
