...
IUSE="acl +daemon debug doc elogind +introspection lvm nls selinux smart systemd +tmpfiles udev"
...
src_install() {
	...
	if ! use doc; then
		rm -r "${ED}"/usr/share/gtk-doc
	fi
	if ! use tmpfiles; then
		rm -r "${ED}"/usr/lib/tmpfiles.d
	fi
	...
	# bug #865663
	use tmpfiles && dotmpfiles data/tmpfiles.d/udisks2.conf
}
