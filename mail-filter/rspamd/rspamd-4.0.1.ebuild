...
SLOT="0"
IUSE="blas curl +hyperscan jemalloc +jit selinux snowball systemd test +tmpfiles"
RESTRICT="!test? ( test )"
...
RDEPEND="
	...
	snowball? ( dev-libs/snowball-stemmer:= )
"
DEPEND="
	${RDEPEND}
	dev-cpp/doctest
...
src_prepare() {
	...
	rm -vrf contrib/{doctest,lua-{argparse,bit},snowball,xxhash,zstd} ||
		die

	> cmake/Toolset.cmake || die #827550

	sed -i -e 's/User=_rspamd/User=rspamd/g' rspamd.service ||
		die
}
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DENABLE_PCRE2=ON
		-DENABLE_SNOWBALL=$(usex snowball ON OFF)
		-DENABLE_URL_INCLUDE=$(usex curl ON OFF)
...
src_install() {
	...
	use systemd && systemd_newunit rspamd.service rspamd.service

	use tmpfiles && newtmpfiles "${FILESDIR}"/${PN}.tmpfile ${PN}.conf
