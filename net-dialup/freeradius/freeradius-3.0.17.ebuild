...
inherit autotools pam prefix python-single-r1 systemd user
...
# NOTE: Temporarily freeradius doesn't support linking with mariadb client
...
pkg_config() {
	if use ssl; then
		...
		use prefix || chown -R root:radius "${ROOT}"/etc/raddb/certs
	fi
}
