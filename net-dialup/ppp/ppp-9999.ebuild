...
inherit eutils git-2 multilib toolchain-funcs linux-info pam

DESCRIPTION="Point-to-Point Protocol (PPP)"
HOMEPAGE="http://www.samba.org/ppp"
SRC_URI=""
EGIT_REPO_URI="git://git.ozlabs.org/~paulus/ppp.git"

LICENSE="BSD GPL-2"
SLOT="0"
KEYWORDS=""
IUSE="activefilter atm gtk ipv6 pam radius"
...
src_install() {
	...
	for i in ip-up ip-down ; do
		newexe "${WORKDIR}/${P}/scripts/${i}.local.add" "${i}.local"
		use ipv6 && newexe "${WORKDIR}/${P}/scripts/${i/ip/ipv6}.sample" "${i/ip/ipv6}.local"
	done
...
pkg_postinst() {
	if linux-info_get_any_version && linux_config_src_exists ; then
		...
		local WARNING_PACKET="CONFIG_PACKET:\t missing AF_PACKET support (optional, used by rp-pppoe plugin)"
		if use atm ; then
