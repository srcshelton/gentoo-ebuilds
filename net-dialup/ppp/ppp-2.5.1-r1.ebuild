...
inherit autotools flag-o-matic linux-info pam tmpfiles
...
IUSE="activefilter atm gtk pam radius selinux systemd +tmpfiles"
...
src_prepare() {
	if ! use radius; then
		sed '|pppd/plugins/radius/Makefile|d' configure.ac
	fi
	...
	# Use the correct path for lock-files
	sed -i pppd/pathnames.h \
		-e '/^#define PPP_PATH_LOCKDIR/s|PPP_PATH_VARRUN|LOCALSTATEDIR|' ||
		die "'/var/lock' path update failed: ${?}"
}
...
src_install() {
	...
	use tmpfiles && newtmpfiles "${FILESDIR}/pppd.tmpfiles-r1" pppd.conf
...
pkg_postinst() {
	use tmpfiles && tmpfiles_process pppd.conf
}
