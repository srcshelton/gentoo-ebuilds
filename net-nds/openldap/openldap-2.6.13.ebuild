...
inherit autotools flag-o-matic multilib preserve-libs ssl-cert systemd tmpfiles toolchain-funcs verify-sig multilib-minimal
...
IUSE="argon2 autoca +cleartext crypt cxx debug experimental gnutls iodbc kerberos kinit minimal odbc overlays pbkdf2 perl samba sasl selinux sha2 smbkrb5passwd ssl static-libs +syslog systemd tcpd test"
REQUIRED_USE="
	cxx? ( sasl )
	pbkdf2? ( ssl )
...
multilib_src_configure() {
	...
	sed -i \
		-e 's:^runstatedir=.*:runstatedir=${EPREFIX}/var/run:' \
		configure contrib/ldapc++/configure contrib/ldaptcl/configure || die 'could not set runstatedir'

	sed -i \
		-e "s:/var/run/sasl2/mux:${EPREFIX}/var/run/sasl2/mux:" \
...
multilib_src_install() {
	...
	if ! use minimal && multilib_is_native_abi; then
		...
		if use systemd; then
			...
			use tmpfiles && newtmpfiles "${FILESDIR}"/slapd.tmpfilesd slapd.conf
		fi
...
pkg_postinst() {
	if ! use minimal ; then
		if use systemd && use tmpfiles; then
