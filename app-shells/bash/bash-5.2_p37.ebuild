...
IUSE="afs bashlogger examples mem-scramble +net nls pgo plugins +readline"
...
src_prepare() {
	...
	sed -i -E '/^(HS|RL)USER/s:=.*:=:' doc/Makefile.in || die
	touch -r . doc/* || die
...
Files located under /etc/bash/bashrc.d must now have a suffix of .sh or .bash.
