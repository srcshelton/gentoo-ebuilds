...
SRC_URI="mirror://gentoo/${MY_P}.tar.gz
	mirror://gnu-alpha/${MY_PN}/${MY_P}.tar.gz
	mirror://gentoo/splash.xpm.gz
	mirror://gentoo/${MY_P}-patches-${PATCHVER}.tar.bz2
	https://dev.gentoo.org/~whissi/dist/${MY_PN}/${MY_P}-patches-${PATCHVER}.tar.bz2"
...
src_install() {
	...
	[[ -n ${GRUB_STATIC_PACKAGE_BUILDING} ]] && \
		mv "${D}"/usr/share/doc/{${MY_PF},grub-static-${MY_PF/grub-}}
...
setup_boot_dir() {
	...
	if [[ ! -e ${dir}/grub.conf ]] ; then
		s="${ROOT}/usr/share/doc/${MY_PF}/grub.conf.gentoo"
