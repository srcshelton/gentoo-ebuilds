...
src_configure() {
	...
	# FEATURE_SH_STANDALONE from being used as system bin/sh - so let's
...
pkg_postinst() {
	...
	if use make-symlinks ; then
		...
		# Use --update=none from coreutils-9.2 instead of -n,
		# add '|| die'
		# Skip legacy linuxrc link, if anyone really needs it they can
		# create it manually
		cp -vpP --update=none _install/bin/* "${ROOT}"/bin/ || die
		cp -vpP --update=none _install/sbin/* "${ROOT}"/sbin/ || die
		cp -vpP --update=none _install/usr/bin/* "${ROOT}"/usr/bin/ ||
			die
	fi
