...
EAPI=8
EGIT_COMMIT="422346057b7041468eb5c894ba0829bdbc6f4280"
EGIT_REPO="utils"
...
src_prepare() {
	...
	sed -e '/STATIC/ s|${STATIC}|STATIC|' \
		-i dtmerge/CMakeLists.txt
...
src_install() {
	...
	# dtmerge and dtoverlay are being linked to libdtovl.so which is provided
	# by media-libs/raspberrypi-userland and also built by this source, but
	# never installed.
	#
	# See https://github.com/raspberrypi/utils/issues/103
	#
	#if [[ -e "${BUILD_DIR}/dtmerge/libdtovl.so" ]]; then
	#	dolib.so "${BUILD_DIR}/dtmerge/libdtovl.so"
	#fi
	#
	# Update: Changing '${STATIC}' to 'STATIC' also resolves this issue.
