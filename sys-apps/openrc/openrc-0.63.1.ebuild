...
src_prepare() {
	...
	if ! use vanilla; then
		...
		eapply "${FILESDIR}/${PN}-0.63-init.d.patch" || die "init.d eapply failed"
		eapply "${FILESDIR}/${P}-net-online.patch" || die "net-online eapply failed"
	fi
}
...
src_configure() {
	...
	if use kernel_Hurd ; then
		...
		emesonargs+=(
			--libexecdir="${EPREFIX}/usr/libexec/openrc"
		)
	fi
