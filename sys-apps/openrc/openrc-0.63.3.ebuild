...
inherit branding eapi9-ver meson pam usr-ldscript
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-0.63.1-checkpath-mkdir.patch"
	"${FILESDIR}/${PN}-0.63-functions.sh.patch"
)
...
src_prepare() {
	...
	if ! use vanilla; then
		if use varrun ; then
			eapply "${FILESDIR}/${PN}-0.63.1-norun.patch" ||
				die "no-run eapply failed"
		else
			eapply -p0 -- "${FILESDIR}/${PN}-0.12.4-bootmisc.in.patch" ||
				die "bootmisc.in eapply failed"
		fi
		#eapply "${FILESDIR}/${PN}-0.46-init.patch" ||
		#	die "init eapply failed"
		#eapply -p0 -- "${FILESDIR}/${PN}-0.18.4-devfs.patch" ||
		#	die "devfs eapply failed"
		#eapply -p0 -- "${FILESDIR}/${PN}-0.19.1-functions.sh.in.patch" ||
		#	die "functions.sh.in eapply failed"
		eapply "${FILESDIR}/${PN}-0.46-rc.conf.patch" ||
			die "rc.conf.in eapply failed"
		#eapply "${FILESDIR}/${PN}-0.48-init.d.patch" ||
		#	die "init.d eapply failed"
		eapply "${FILESDIR}/${PN}-0.48-keymaps.patch" ||
			die "keymaps.in eapply failed"
		eapply "${FILESDIR}/${PN}-0.63-init.d.patch" ||
			die "init.d eapply failed"
		eapply "${FILESDIR}/${PN}-0.63.1-net-online.patch" ||
			die "net-online eapply failed"
	fi
}
