...
LICENSE="BSD-2"
SLOT="0"
IUSE="audit bash compat debug ncurses +netifrc newnet pam prefix selinux static-libs sysv-utils +tmpfiles unicode -vanilla +varrun"
...
src_prepare() {
	...
	if ! use vanilla; then
		if use varrun ; then
			eapply -p0 -- "${FILESDIR}/openrc-0.38.2-norun.patch" || die "eapply failed"
		else
			eapply -p0 -- "${FILESDIR}/openrc-0.12.4-bootmisc.in.patch" || die "bootmisc.in eapply failed"
		fi
		eapply -p0 -- "${FILESDIR}/openrc-0.38.2-init.patch" || die "init eapply failed"
	...
	eapply "${FILESDIR}/openrc-0.41.2-cgroup-race.patch" || die "cgroup eapply failed"
