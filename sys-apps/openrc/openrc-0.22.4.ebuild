...
# $Id: 18e92d39d9ec9581ca9872641286841529d71fad $
...
IUSE="audit debug ncurses pam newnet prefix +netifrc selinux static-libs
	tools unicode -vanilla +varrun kernel_linux kernel_FreeBSD"
...
src_prepare() {
	...
	if ! use vanilla; then
		if use varrun ; then
			eapply -p0 -- "${FILESDIR}/openrc-0.22.4-norun.patch" || die "eapply failed"
		else
		...
		eapply -p0 -- "${FILESDIR}/openrc-0.22.4-rc.conf.patch" || die "rc.conf.in eapply failed"
	fi
