...
# $Id: e05995db793f8c8911bdd4a1e5a0a503b79b1a80 $
...
IUSE="debug ncurses pam newnet prefix +netifrc selinux static-libs
	+tmpfs tools unicode +varrun kernel_linux kernel_FreeBSD"
...
src_prepare() {
	...
	if use varrun ; then
		epatch "${FILESDIR}/openrc-0.13.7-norun.patch" || die "epatch failed"
	else
		epatch "${FILESDIR}/openrc-0.12.4-bootmisc.in.patch" || die "bootmisc.in epatch failed"
	fi
	use tmpfs && epatch "${FILESDIR}/openrc-0.12.4-tmpfs.patch" || die "tmpfs epatch failed"
...
src_install() {
	...
	newinitd "${FILESDIR}"/localmount-r1.initd localmount
