...
# $Id: a2474e8118b7d46338fb0d8d0621978816cca4e3 $
...
src_prepare() {
	...
	if use vanilla; then
		...
		if use varrun ; then
			eapply -p0 -- "${FILESDIR}/openrc-0.16.4-norun.patch" || die "eapply failed"
		else
			eapply -p0 -- "${FILESDIR}/openrc-0.12.4-bootmisc.in.patch" || die "bootmisc.in eapply failed"
		fi
		if use tmpfs; then
			eapply -p0 -- "${FILESDIR}/openrc-0.12.4-tmpfs.patch" || die "tmpfs eapply failed"
		fi
