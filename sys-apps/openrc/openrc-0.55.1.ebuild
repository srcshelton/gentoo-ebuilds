...
LICENSE="BSD-2"
SLOT="0"
IUSE="audit bash caps compat debug +netifrc newnet pam s6 selinux +sysvinit sysv-utils +tmpfiles unicode -vanilla +varrun"
...
src_prepare() {
	...
	if ! use vanilla; then
		...
		eapply "${FILESDIR}/${P}-init.d.patch" || die "init.d eapply failed"
