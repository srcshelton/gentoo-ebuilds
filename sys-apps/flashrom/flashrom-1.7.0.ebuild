...
IUSE_PROGRAMMERS="
	...
	+stlinkv3-spi
	+usbblaster-spi"
IUSE="+asm106x atahpt atapromise +atavia +buspirate-spi +ch341a-spi +ch347-spi +dediprog +developerbox-spi +digilent-spi +dirtyjtag-spi +drkaiser +dummy +ft2232-spi +gfxnvidia +internal +internal-dmi +it8212 ${IUSE_PROGRAMMERS} +jlink-spi +linux-mtd +linux-spi mediatek-i2c-spi mstarddc-spi +nic3com +nicintel +nicintel-eeprom +nicintel-spi nicnatsemi +nicrealtek +nv-sma-spi +ogp-spi parade-lspcon +pickit2-spi +pony-spi +raiden-debug-spi +rayer-spi realtek-mst-i2c-spi +satamv +satasii +serprog +spidriver +stlinkv3-spi test tools +usbblaster-spi"
...
src_install() {
	...
	# the classic CLI executable uses internal symbols from that library.
	# Therefore, we let it be built but keep it out of the installed tree.
