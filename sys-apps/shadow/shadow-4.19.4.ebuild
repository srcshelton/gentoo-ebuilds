...
inherit libtool pam user-info usr-ldscript verify-sig
...
SRC_URI="https://github.com/shadow-maint/shadow/releases/download/${MY_PV}/${MY_P}.tar.xz
	verify-sig? ( https://github.com/shadow-maint/shadow/releases/download/${MY_PV}/${MY_P}.tar.xz.asc )"
S="${WORKDIR}/${MY_P}"
...
BDEPEND="${BDEPEND} verify-sig? ( >=sec-keys/openpgp-keys-alejandro-colomar-20260122 )"
...
src_install() {
	...
	if use split-usr ; then
		...
		# By default, /bin/getsubids depends on
		# /usr/lib64/libsubid.so.4 - we could move the binary to /usr/bin, but it's
		# probably safer to move the library to the root filesystem...
...
pkg_postinst() {
	...
	if [[ "${ROOT:-"/"}" != '/' ]]; then
		...
		if group="$(egetent group shadow | cut -d: -f3)" &&
				[[ -n "${group:-}" ]]
		then
			chgrp "${group}" "${EROOT}"/usr/bin/{chage,expiry}
		else
			chgrp "${SHADOW_GID}" "${EROOT}"/usr/bin/{chage,expiry}
		fi
