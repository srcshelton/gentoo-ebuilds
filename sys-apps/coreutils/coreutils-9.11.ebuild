...
LICENSE="GPL-3+"
SLOT="0"
IUSE="acl caps gmp hostname kill multicall nls +openssl selinux +split-usr static +stdbuf test test-full uptime vanilla xattr"
RESTRICT="!test? ( test )"
...
src_prepare() {
	...
	sed -e "s/libstdbuf\\.so/libstdbuf$(get_libname)/" -i \
			src/stdbuf.c \
			Makefile.in \
			configure ||
		die
...
src_configure() {
	...
	local myconf=(
		...
		# kill/uptime - procps
		# groups/su   - <sys-apps/shadow-4.19.0
		# hostname    - net-tools
		--enable-install-program="arch,$(usev hostname),$(usev kill),$(usev stdbuf),$(usev uptime)"
		--enable-no-install-program="$(usev !hostname),$(usev !kill),$(usev !stdbuf),su,$(usev !uptime)"  # groups
