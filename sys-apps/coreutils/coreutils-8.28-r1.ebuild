...
IUSE="acl caps elibc_Cygwin elibc_glibc gmp hostname kill multicall nls selinux sep-usr static +stdbuf uptime test userland_BSD vanilla xattr"
...
src_install() {
	...
	if [[ ${USERLAND} == "GNU" ]] ; then
		...
		if use kill; then
			mv kill ../../bin/ || die "Could not move 'kill' binary from /usr/bin to /bin"
		fi
