...
src_install() {
	...
	# man-page states:
	#
	#   it is important that the file is available from earliest boot on,
	...
	if ! [[ -e "${ED}"/usr/lib/os-release ]]; then
		dosym -r /etc/os-release /usr/lib/os-release
	fi
...
pkg_preinst() {
	...
	if use varrun; then
		if [[ ! -d "${EROOT}/var/lock" ]]; then
			dodir /var/lock
		fi
		if [[ ! -d "${EROOT}/var/run" ]]; then
			dodir /var/run
		fi
	else
...
pkg_postinst() {
	...
	if use kernel_linux; then
		use varrun || mkdir -p "${EROOT}"/run || die
