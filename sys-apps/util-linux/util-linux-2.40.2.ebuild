...
SLOT="0"
IUSE="audit build caps +cramfs cryptsetup fdformat +hardlink kill +logger magic ncurses nls pam python +readline rtas selinux slang split-usr static-libs +su +suid systemd test tty-helpers udev unicode uuidd"
...
"
# Something appears to be (more) broken in sys-apps/portage-3.0.67 - firstly,
...
multilib_src_configure() {
	...
	if multilib_is_native_abi ; then
		myeconfargs+=(
			...
			$(use_with tmpfiles tmpfilesdir "${EPREFIX}/usr/lib/tmpfiles.d")
