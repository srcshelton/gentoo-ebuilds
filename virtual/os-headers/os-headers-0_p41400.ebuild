...
SLOT="41400"
# KEYWORDS should match sys-kernel/linux-headers-4.14*
...
RDEPEND="
	!prefix-guest? (
		kernel_linux? ( || (
			arm? (
				cix? ( >=sys-kernel/cix-headers-4.14:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-4.14:0 )
				rockchip? ( >=sys-kernel/rockchip-headers-4.14:0 )
			)
			arm64? (
				cix? ( >=sys-kernel/cix-headers-4.14:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-4.14:0 )
			...
			>=sys-kernel/linux-headers-4.14:0
		) )
		!kernel_linux? (
