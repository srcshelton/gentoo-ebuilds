...
SLOT="41500"
# KEYWORDS should match sys-kernel/linux-headers-4.15*
...
RDEPEND="
	!prefix-guest? (
		kernel_linux? ( || (
			arm? (
				cix? ( >=sys-kernel/cix-headers-4.15:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-4.15:0 )
				rockchip? ( >=sys-kernel/rockchip-headers-4.15:0 )
			)
			arm64? (
				cix? ( >=sys-kernel/cix-headers-4.15:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-4.15:0 )
			...
			>=sys-kernel/linux-headers-4.15:0
		) )
		!kernel_linux? (
