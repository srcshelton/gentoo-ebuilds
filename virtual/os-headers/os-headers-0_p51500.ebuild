...
SLOT="51500"
# KEYWORDS should match sys-kernel/linux-headers-5.15*
...
RDEPEND="
	!prefix-guest? (
		kernel_linux? ( || (
			arm? (
				cix? ( >=sys-kernel/cix-headers-5.15:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-5.15:0 )
				rockchip? ( >=sys-kernel/rockchip-headers-5.15:0 )
			)
			arm64? (
				cix? ( >=sys-kernel/cix-headers-5.15:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-5.15:0 )
			...
			>=sys-kernel/linux-headers-5.15:0
		) )
		!kernel_linux? (
