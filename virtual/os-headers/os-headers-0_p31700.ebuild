...
SLOT="31700"
# KEYWORDS should match sys-kernel/linux-headers-3.17*
...
RDEPEND="
	!prefix-guest? (
		kernel_linux? ( || (
			arm? (
				cix? ( >=sys-kernel/cix-headers-3.17:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-3.17:0 )
				rockchip? ( >=sys-kernel/rockchip-headers-3.17:0 )
			)
			arm64? (
				cix? ( >=sys-kernel/cix-headers-3.17:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-3.17:0 )
			...
			>=sys-kernel/linux-headers-3.17:0
		) )
		!kernel_linux? (
