...
SLOT="20600"
# KEYWORDS should match sys-kernel/linux-headers-2.6*
...
RDEPEND="
	!prefix-guest? (
		kernel_linux? ( || (
			arm? (
				cix? ( >=sys-kernel/cix-headers-2.6:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-2.6:0 )
				rockchip? ( >=sys-kernel/rockchip-headers-2.6:0 )
			)
			arm64? (
				cix? ( >=sys-kernel/cix-headers-2.6:0 )
				raspberrypi? ( >=sys-kernel/raspberrypi-headers-2.6:0 )
