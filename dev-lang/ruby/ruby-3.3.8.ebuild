...
inherit autotools flag-o-matic multiprocessing rust toolchain-funcs
...
src_configure() {
	...
	# Some CFLAGS/CXXFLAGS directive is causing an internal compiler error with
	# gcc 14.2.1.1868_p20241221 p7 which doesn't affect ruby:3.2 and goes away
	# with more conservative FLAGS...
	#
	# Unable to coalesce ssa_names 32 and 34 which are marked as MUST COALESCE.
	# _ec_32(ab) and  _ec_34(ab)
	# during RTL pass: expand
	# ractor.c: In function 'ractor_check_ints.constprop':
	# ractor.c:583:1: internal compiler error: SSA corruption
	#
	if [[ "$(gcc-fullversion)" == 14.2.1 ]]; then
		filter-lto
	fi
