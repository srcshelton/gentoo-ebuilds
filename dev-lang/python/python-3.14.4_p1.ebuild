...
src_configure() {
	...
	# -ffast-math causes CPython to hang when building numpy
...
src_compile() {
	...
	# CPython requires IEEE-754 infinities to retain their defined semantics.
	# Keep this unconditional since src_test is controlled by USE=test.
	if ! tc-is-cross-compiler; then
		if ! LD_LIBRARY_PATH="${S}${LD_LIBRARY_PATH:+":${LD_LIBRARY_PATH}"}" \
				timeout --signal=KILL 10 ./python - <<-'PYTHON'
			import math
			import sys

			value = float("inf")
			if math.isfinite(value):
			    raise SystemExit("math.isfinite(inf) returned true")
			if hash(value) != sys.hash_info.inf:
			    raise SystemExit("hash(inf) returned an invalid value")
			if hash(-value) != -sys.hash_info.inf:
			    raise SystemExit("hash(-inf) returned an invalid value")

			# Compilation hashes constants and exposed the original infinite loop.
			compile(
			    "value = 1e1000",
			    "<non-finite-float-validation>",
			    "exec",
			)
			PYTHON
		then
			die "python failed non-finite floating-point validation"
		fi
	fi
}
