...
inherit flag-o-matic multilib preserve-libs toolchain-funcs usr-ldscript verify-sig multilib-minimal
...
if [[ ${PV} == *_p* ]] ; then
	...
	if [[ -z ${PATCH_DATES[@]} ]] ; then
		SRC_URI+=" https://invisible-island.net/archives/${PN}/${PV/_p*}/${MY_P/_p/-}.patch.sh.gz
			verify-sig? ( https://invisible-island.net/archives/${PN}/${PV/_p*}/${MY_P/_p/-}.patch.sh.gz.asc"
		...
		# Repeat for .patch.gz.asc for verify-sig, and for all of the URLs,
		# chuck in invisible-island.net too:
		SRC_URI+=" ${mangled_patches[@]}
			${mangled_patches[@]/${upstream_url_base}/${upstream_m_url_base}}
			verify-sig? ( ${mangled_patches_sig[@]} )
			verify-sig? ( ${mangled_patches_sig[@]/${upstream_url_base}/${upstream_m_url_base}} )"
