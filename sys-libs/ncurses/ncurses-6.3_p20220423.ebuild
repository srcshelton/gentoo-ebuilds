...
if [[ "${PV}" == *_p* ]] ; then
	...
	if [[ -z ${PATCH_DATES[@]} ]] ; then
		SRC_URI+=" https://invisible-island.net/archives/${PN}/${PV/_p*}/${P/_p/-}.patch.sh.gz
			verify-sig? ( https://invisible-island.net/archives/${PN}/${PV/_p*}/${P/_p/-}.patch.sh.gz.asc"
	else
		patch_url=
		my_patch_index=
		for (( my_patch_index=0 ; my_patch_index < ${#PATCH_DATES[@]} ; my_patch_index++ )); do
			patch_url="https://invisible-island.net/archives/${PN}/${PV/_p*}/${MY_P}-${PATCH_DATES[${my_patch_index}]}.patch.gz"
			SRC_URI+=" ${patch_url}
				verify-sig? ( ${patch_url}.asc )"
		done
		unset patch_url
