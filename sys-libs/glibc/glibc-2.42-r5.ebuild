...
SLOT="2.2"
IUSE="audit caps cet compile-locales -custom-cflags doc gd hash-sysv-compat headers-only -minimal +multiarch multilib multilib-bootstrap nscd perl profile selinux sframe +ssp stack-realign +static-libs suid systemd systemtap test -timezone-tools +tmpfiles vanilla x32"
...
do_compile_test() {
	local ret=1 save_cflags="${CFLAGS}"
	if [[ "${1}" != "${CFLAGS}" ]]; then
		CFLAGS+=" ${1}"
	fi
	shift
...
setup_env() {
	...
	if is_crosscompile; then
		...
		# when ${CTARGET}-g++ is not yet available. We avoid
...
src_prepare() {
	...
	if use x32 && [[ "${ARCH}" == "amd64" && "$( get_abi_LIBDIR x32 )" != 'libx32' ]]; then
...
glibc_do_src_install() {
	...
	if use nscd ; then
		...
		# The provided 'nscd-1' includes neither templated prefix lines nor any
		# mention of '@PIDFILE@'!
		#
