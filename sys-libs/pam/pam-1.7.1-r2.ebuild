...
inherit db-use fcaps flag-o-matic meson-multilib toolchain-funcs user-info usr-ldscript
...
multilib_src_install() {
	...
	if use split-usr; then
		...
		#ls -l "${ED}/$(get_libdir)/" "${ED}/usr/$(get_libdir)/"
	fi
}
...
pkg_postinst() {
	...
	if [[ -n "${ROOT:-}" ]]; then
		...
		if gid="$(egetent group shadow | cut -d: -f3)" &&
				[[ -n "${gid:-}" ]]
		then
