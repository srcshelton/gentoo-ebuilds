...
inherit edo flag-o-matic linux-info multibuild multilib multilib-build multiprocessing preserve-libs sysroot toolchain-funcs usr-ldscript
...
src_configure() {
	...
	tc-is-clang && append-flags $(
		test-flags-CC -Wno-error=unused-command-line-argument
	)
...
openssl_src_install() {
	...
	if use split-usr && openssl_is_default_variant ; then
		# need the libs in / (libssl.so for sys-fs/cryptsetup)
		gen_usr_ldscript -a crypto ssl
	fi
}
