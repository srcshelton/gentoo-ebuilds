...
inherit autotools db-use edos2unix flag-o-matic java-pkg-opt-2 multilib pam systemd tmpfiles toolchain-funcs multilib-minimal
...
IUSE="authdaemond berkdb gdbm kerberos ldapdb mysql openldap pam postgres sample selinux sqlite srp ssl static-libs systemd urandom"
...
pkg_config() {
	...
	if [[ ! -f "${EROOT}/etc/sasl2/sasldb2" ]] ; then
		...
		use prefix || chown root:mail "${EROOT}/etc/sasl2/sasldb2" \
