...
src_prepare() {
	...
	if use cacert ; then
		eapply -p2 "${DISTDIR}/${PN}-3.101-cacert-class1-class3.patch"
	fi
...
multilib_src_configure() {
	...
	sed -i \
		-e "/@libdir@/ s:lib64:$(get_libdir):" config/Makefile || die
}
...
nssbits() {
	...
	case "$(file -S "${T}/${1}test.o")" in
		...
		*32-bit*|*ppc*|*i386*)		: ;;
		*)							die "Failed to detect whether ${cc}" \
										"builds 64bits or 32bits, disable" \
...
multilib_src_compile() {
	...
	case "${ABI}" in
		...
		${DEFAULT_ABI})	einfo "Running compilation test to determine bit'ness"
						mybits="$(nssbits)"
						;;
	esac
...
multilib_src_test() {
	...
	OBJDIR="${objdir}" \
	DIST="${BUILD_DIR}/dist" \
	MOZILLA_ROOT="${BUILD_DIR}" ./all.sh || die
}
...
multilib_src_install() {
	...
	# create nss-softokn.pc from nss.pc for libfreebl and some private headers
	# bug 517266
	sed  \
