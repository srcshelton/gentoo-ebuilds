...
# $Id: 4e48aa6f3b8f23dcd3547140fbfcc549c1c56434 $
...
pkg_setup() {
	...
	if use rules; then
		# We don't want to include the emerging rules download in SRC_URI,
		# above, since it is a rapidly changing snapshot.  Instead, let's fetch
		# it on each package build...
		ebegin "Fetching additional rules files"
		(
			DISTDIR="${T}"
			FILE="emerging.rules.tar.gz"
			URI="https://rules.emergingthreats.net/open/suricata/emerging.rules.tar.gz"
			FETCHCOMMAND="${FETCHCOMMAND:-$( grep '^\s*FETCHCOMMAND=' "${EROOT:-}/etc/portage/make.conf" | tail -n 1 )}"
			FETCHCOMMAND="${FETCHCOMMAND:-wget -t 3 -T 60 --passive-ftp -O \"${DISTDIR}/${FILE}\" \"${URI}\"}"
			eval einfo "About to run '${FETCHCOMMAND}' ..."
			eval ${FETCHCOMMAND} && tar -xzvpf "${DISTDIR}/${FILE}" -C "${DISTDIR}"
		)
		eend ${?} "Unable to fetch emerging rules"
	fi
}
