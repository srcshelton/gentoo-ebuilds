...
IUSE="apache asterisk cgi dhcpd doc http ipmi ipv6 irc java memcached minimal mysql postgres selinux ssl syslog systemd test"
...
src_install() {
	...
	emake -j1 DESTDIR="${ED}" $(usex minimal "install-minimal install-man" install)
	...
	if use minimal; then
		...
		if use cgi; then
			...
			sed -e '/#graph_strategy cgi/s:#graph_strategy cgi:graph_strategy cron:' \
...
pkg_postinst() {
	...
	if use cgi; then
		...
		#	"${EROOT}"/var/cache/munin-cgi \
		#	"${EROOT}"/var/log/munin/munin-cgi-{graph,html}.log
