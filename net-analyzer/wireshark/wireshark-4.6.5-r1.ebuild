...
inherit cmake fcaps lua-single python-any-r1 qmake-utils toolchain-funcs xdg
...
if [[ ${PV} == *9999* ]] ; then
	...
	SRC_URI="https://www.wireshark.org/download/src/all-versions/${P/_/}.tar.xz
		verify-sig? ( https://www.wireshark.org/download/SIGNATURES-${PV/_/}.txt -> ${P}-signatures.txt )"
	S="${WORKDIR}/${P/_/}"
...
LICENSE="GPL-2"
SLOT="0/${PV}"
IUSE="androiddump bcg729 brotli +capinfos +captype ciscodump crypt +dftest doc dpauxmon +dumpcap +editcap -gui http2 http3 ilbc kerberos lua lz4 maxminddb +mergecap +minizip +netlink opus +pcap pkcs11 +plugins +randpkt +randpktdump +reordercap sbc sdjournal selinux +sharkd smi snappy spandsp sshdump ssl test +text2pcap +tshark +udpdump wifi xxhash zlib +zstd"
...
src_prepare() {
	# since 4.6.5 the Lua version is found "automatically" and can no
	# longer be passed in via LUA_FIND_VERSIONS, so we override the search
	# list.
	if use lua; then
