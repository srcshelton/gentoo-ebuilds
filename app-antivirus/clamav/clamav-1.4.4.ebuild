	...
	if ! use libclamav-only ; then
		...
		if use clamapp ; then
			...
			local clamd_sed_args=(
				-e "s:^\(Example\):\# \1:"
				-e "s:^#\(PidFile\) .*:\1 ${EPREFIX}/var/run/clamd.pid:"
				-e "s/^#\(User .*\)/\1/"
			...
			if use systemd; then
				clamd_sed_args+=(
					-e "s:^#\(LocalSocket \)/run.*:\1${EPREFIX}/var/run/clamav/clamd.ctl:"
				)
			else
				clamd_sed_args+=(
					# There's one in /tmp that we don't want to default-enable
					-e "s:^#\(LocalSocket \(/var\)\?/run.*\):\1:"
				)
			fi
			...
			local freshclam_sed_args=(
				-e "s:^\(Example\):\# \1:"
				-e "s:^#\(PidFile\) .*:\1 ${EPREFIX}/var/run/freshclam.pid:"
				-e "s/^#\(DatabaseOwner .*\)/\1/"
			...
			if use milter ; then
				...
				local milter_sed_args=(
					-e "s:^\(Example\):\# \1:"
					-e "s:^\#\(PidFile\) .*:\1 ${EPREFIX}/var/run/clamav-milter.pid:"
